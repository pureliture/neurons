...
def index_json(base_url, api_key, method_path, payload=None, timeout=30):
    headers = {"Authorization": f"Bearer {api_key}"}
    response = http_json(base_url, method_path, payload, headers=headers, timeout=timeout)
    if response.get("code", 0) != 0:
        raise RuntimeError("retired_index_bridge api returned non-zero code")
...
def find_document(base_url, api_key, dataset_id, filenames):
    ...
    data = index_json(base_url, api_key, f"/api/v1/datasets/{urllib.parse.quote(dataset_id)}/documents?{query}")
    docs = data.get("docs", data) if isinstance(data, dict) else data
    if not isinstance(docs, list):
        raise RuntimeError("retired_index_bridge document list response missing docs")
    for doc in docs:
...
def retrieve(base_url, api_key, dataset_id, marker, *, project, metadata_filter=True):
    ...
    data = index_json(base_url, api_key, "/api/v1/retrieval", payload)
...
def main():
    ...
    parser.add_argument("--retired-index-bridge-url", default=os.environ.get("RETIRED_INDEX_BRIDGE_VERIFY_BASE_URL", "http://127.0.0.1:9380"))
    parser.add_argument("--index-api-key", default=os.environ.get("RETIRED_INDEX_BRIDGE_API_KEY", ""))
    parser.add_argument("--dataset-id", default=os.environ.get("RETIRED_INDEX_BRIDGE_TRANSCRIPT_MEMORY_DATASET_ID", ""))
    parser.add_argument("--timeout", type=int, default=300)
    parser.add_argument("--ledger", default="build/private-live-ledger/ledger.sqlite")
    parser.add_argument("--evidence", default="build/reports/rag-ingress-queue/live-index-verify.json")
    ...
    if not args.index_api_key:
        raise RuntimeError("RETIRED_INDEX_BRIDGE_API_KEY is required")
    if not args.dataset_id:
        raise RuntimeError("RETIRED_INDEX_BRIDGE_TRANSCRIPT_MEMORY_DATASET_ID is required")
    ...
    body = (
        ...
        "---\n"
        f"{marker} redacted live RetiredIndexBridge write ledger gate recall promote smoke.\n"
    )
    ...
    def indexed_document():
        ...
        if run in {"FAIL", "FAILED", "CANCEL"}:
            raise RuntimeError("RetiredIndexBridge document parsing failed")
        return None

    searchable_chunk_source = "index_parse_done"
    try:
        indexed = wait_until(deadline, indexed_document, "RetiredIndexBridge DONE status")
    except RuntimeError:
    ...
    chunks, authorized_chunks, retrieval_mode = wait_until(
        ...
        "RetiredIndexBridge retrieval of authorized document",
    )
    ...
    evidence = {
        "runtime": {
            ...
            "scope": (
                ...
                else "ubuntu-compose-live-index-write-external-authorization-recall-promote"
            ),
