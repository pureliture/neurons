 ...
 * back-to-back re-deliveries dedup even before RetiredIndexBridge's document search index reflects
 ...
 * <ul>
 *   <li>Optimistic: a finalized hit short-circuits to "already delivered" without re-checking RetiredIndexBridge,
 ...
 *       retired-index-bridge lag if external deletes are a concern.</li>
 ...
 *       be matched by name and treated as present. Closing that gap would require querying RetiredIndexBridge run
...
class RecentDeliveryCache {
     ...
     * @param documentId the uploaded RetiredIndexBridge document id used to resume post-upload steps on retry;
    ...
    private static String key(String datasetId, String fragment) {
        // datasetId and fragment are RetiredIndexBridge hex identifiers, so '|' is a collision-free delimiter.
        // datasetId is guaranteed non-blank by RetiredIndexBridgeTargetAdapter#isConfigured before the cache is
