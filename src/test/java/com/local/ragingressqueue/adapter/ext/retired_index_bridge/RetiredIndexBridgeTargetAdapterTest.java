...
class RetiredIndexBridgeTargetAdapterTest {
    @Test
    void disabledAdapterReportsClosedPressureAndDoesNotClaimDelivery() {
        RetiredIndexBridgeTargetAdapter adapter = new RetiredIndexBridgeTargetAdapter(false);

        assertThat(adapter.pressureSnapshot("index-transcript-memory").pressure()).isEqualTo(TargetPressure.CLOSED);
        assertThat(adapter.deliver(validJob(), "index-transcript-memory").delivered()).isFalse();
    }
    ...
    void statusSnapshotDoesNotExposeTargetPrivateReference() {
        ...
        TargetStatusSnapshot snapshot = adapter.getStatus(validJob(), "index-transcript-memory");
    ...
    void configuredAdapterSurfacesBackendNeutralStatusWithoutLeakingResourceIds() {
        RetiredIndexBridgeTargetAdapter adapter = new RetiredIndexBridgeTargetAdapter(
            ...
            "http://127.0.0.1:9380",
            "token",
            Map.of("index-transcript-memory", "ds_1"),
            new FakeRetiredIndexBridgeGateway()
        );
    ...
    void enabledAdapterUploadsMetadataAndRequestsParse() {
        ...
        DeliveryResult result = adapter.deliver(job, "index-transcript-memory");
        ...
        assertThat(gateway.metadata).containsEntry("project", "workspace-index-advisor");
    ...
    void secondDeliveryWithSameContentHashIsSkippedAsAlreadyPresent() {
        ...
        DeliveryResult first = adapter.deliver(job, "index-transcript-memory");
        assertThat(first.delivered()).isTrue();
        assertThat(gateway.uploadCount).isEqualTo(1);

        DeliveryResult second = adapter.deliver(job, "index-transcript-memory");
        assertThat(second.delivered()).isTrue();
        assertThat(gateway.uploadCount).isEqualTo(1);
    }

    @Test
    void reDeliverySkipsUploadEvenWhenRetiredIndexBridgeStillReportsAbsent() {
        // Freshness race: RetiredIndexBridge's document search index may not yet reflect the just-uploaded
        ...
        IngestJob job = validJob();
        assertThat(adapter.deliver(job, "index-transcript-memory").delivered()).isTrue();
        assertThat(adapter.deliver(job, "index-transcript-memory").delivered()).isTrue();
    ...
    void cacheHitSkipsRedundantGatewayLookupOnReDelivery() {
        // Performance: a cached recent delivery must short-circuit before the extra RetiredIndexBridge GET.
        FakeRetiredIndexBridgeGateway gateway = new FakeRetiredIndexBridgeGateway();
        RetiredIndexBridgeTargetAdapter adapter = new RetiredIndexBridgeTargetAdapter(
            true,
            "http://127.0.0.1:9380",
        ...
        IngestJob job = validJob();
        adapter.deliver(job, "index-transcript-memory");
        adapter.deliver(job, "index-transcript-memory");
    ...
    void dedupSkipsIdenticalReDeliveryForNonTranscriptKind() {
        ...
        RetiredIndexBridgeTargetAdapter adapter = new RetiredIndexBridgeTargetAdapter(
            ...
            "http://127.0.0.1:9380",
            "token",
            Map.of("index-session-memory", "ds_session"),
            gateway
        );

        adapter.deliver(sessionMemoryJob(), "index-session-memory");
        adapter.deliver(sessionMemoryJob(), "index-session-memory");
    ...
    void retryAfterPostUploadFailureResumesMetadataAndParseWithoutReUploading() {
        ...
        DeliveryResult retry = adapter.deliver(job, "index-transcript-memory");
    ...
    void retryAfterParseFailureResumesFromParseWithoutReapplyingMetadata() {
        ...
        // redundant RetiredIndexBridge call and contradicts resuming only the step that failed.
        FakeRetiredIndexBridgeGateway gateway = new FakeRetiredIndexBridgeGateway();
        gateway.alwaysReportAbsent = true;
    ...
    @Test
    void existingRetiredIndexBridgeDocumentIsRecordedSoLaterReDeliveriesSkipTheLookup() {
        // First delivery finds the document already present in RetiredIndexBridge (no prior cache entry); the
        // result must be a no-op upload AND the presence must be cached so later re-deliveries skip
        // even the RetiredIndexBridge lookup.
        FakeRetiredIndexBridgeGateway gateway = new FakeRetiredIndexBridgeGateway();
        IngestJob job = validJob();
    ...
    void twoJobsWithDifferentContentHashesAreBothUploaded() {
        ...
        DeliveryResult first = adapter.deliver(jobA, "index-transcript-memory");
        DeliveryResult second = adapter.deliver(jobB, "index-transcript-memory");
    ...
    void enabledAdapterRoutesSessionMemoryProfileToSessionDataset() {
        ...
        DeliveryResult result = adapter.deliver(sessionMemoryJob(), "index-session-memory");

        assertThat(result.delivered()).isTrue();
        assertThat(gateway.uploadDatasetId).isEqualTo("ds_session");
        assertThat(gateway.metadata).containsEntry("target_profile", "index-session-memory");
        assertThat(gateway.metadata).containsEntry("kind", "session_summary");
    }

    @Test
    void enabledAdapterThrottlesWhenRetiredIndexBridgeRunningBacklogReachesLimit() {
        FakeRetiredIndexBridgeGateway gateway = new FakeRetiredIndexBridgeGateway();
        gateway.pressureSnapshot = new RetiredIndexBridgePressureSnapshot(20, 0, 0, 100, 120);
        RetiredIndexBridgeTargetAdapter adapter = new RetiredIndexBridgeTargetAdapter(
            true,
            "http://127.0.0.1:9380",
            "token",
            Map.of("index-transcript-memory", "ds_1"),
            gateway,
            new RetiredIndexBridgePressurePolicy(20, 5, 100, 25)
        );

        assertThat(adapter.pressureSnapshot("index-transcript-memory").pressure()).isEqualTo(TargetPressure.THROTTLED);
    }

    @Test
    void enabledAdapterClosesWhenRetiredIndexBridgeBacklogReachesHardLimit() {
        FakeRetiredIndexBridgeGateway gateway = new FakeRetiredIndexBridgeGateway();
        gateway.pressureSnapshot = new RetiredIndexBridgePressureSnapshot(100, 0, 0, 100, 200);
        RetiredIndexBridgeTargetAdapter adapter = new RetiredIndexBridgeTargetAdapter(
            true,
            "http://127.0.0.1:9380",
            "token",
    ...
    @Test
    void enabledAdapterFailsClosedWhenRetiredIndexBridgePressureCannotBeRead() {
        FakeRetiredIndexBridgeGateway gateway = new FakeRetiredIndexBridgeGateway();
    ...
    @Test
    void enabledAdapterReturnsFailedWhenRetiredIndexBridgeRejectsUpload() {
        FakeRetiredIndexBridgeGateway gateway = new FakeRetiredIndexBridgeGateway();
        ...
        DeliveryResult result = adapter.deliver(validJob(), "index-transcript-memory");

        assertThat(result.delivered()).isFalse();
        assertThat(result.error()).isEqualTo("retired_index_bridge delivery failed");
    ...
    void supersedeEnabledReplacesPriorVersionOfSameLogicalDocumentAndTagsFilename() {
        ...
        assertThat(adapter.deliver(supersedeJob("version one", "L-123"), "index-transcript-memory").delivered()).isTrue();
        assertThat(gateway.uploadFilename).contains("-" + logicalFragment + "-");
        assertThat(gateway.deletedDocumentIds).isEmpty();

        assertThat(adapter.deliver(supersedeJob("version two", "L-123"), "index-transcript-memory").delivered()).isTrue();
    ...
    void supersedeOnlyDeletesPriorVersionsWithTheSameNameNotSiblingsOrForeignDocuments() {
        ...
        adapter.deliver(supersedeJob("version one", "L-123"), "index-transcript-memory");   // doc_1: summary v1
        adapter.deliver(supersedeJob("version two", "L-123"), "index-transcript-memory");   // doc_2: summary v2
    ...
    void supersedeEnabledDoesNotDeleteAnythingWhenNoLogicalIdIsPresent() {
        ...
        adapter.deliver(supersedeJob("chunk one", null), "index-transcript-memory");
        adapter.deliver(supersedeJob("chunk two", null), "index-transcript-memory");
    ...
    void supersedeDisabledByDefaultNeitherTagsTheFilenameNorDeletes() {
        ...
        adapter.deliver(supersedeJob("version one", "L-123"), "index-transcript-memory");
        adapter.deliver(supersedeJob("version two", "L-123"), "index-transcript-memory");
    ...
    void uploadedFilenameIsCanonicalizedSoSupersedeMatchesTheStoredName() {
        // RetiredIndexBridge sanitizes quote/CR/LF in stored names; the adapter must produce the same canonical
    ...
    private static final class FakeRetiredIndexBridgeGateway implements RetiredIndexBridgeGateway {
        ...
        private RetiredIndexBridgePressureSnapshot pressureSnapshot = new RetiredIndexBridgePressureSnapshot(0, 0, 0, 100, 100);
        ...
        public RetiredIndexBridgeDocumentRef uploadDocument(
            ...
            if (failUpload) {
                throw new RetiredIndexBridgeDeliveryException("boom");
            }
            uploadCount++;
            ...
            return new RetiredIndexBridgeDocumentRef(documentId, "UNSTART");
        }
        ...
        public void updateMetadata(String baseUrl, String apiKey, String datasetId, String documentId, Map<String, String> metadata) {
            metadataCallCount++;
            if (failMetadata) {
                throw new RetiredIndexBridgeDeliveryException("metadata boom");
            }
        ...
        public void requestParse(String baseUrl, String apiKey, String datasetId, String documentId) {
            parseCallCount++;
            if (failParse) {
                throw new RetiredIndexBridgeDeliveryException("parse boom");
            }
            parseRequested = true;
        ...
        @Override
        public java.util.List<RetiredIndexBridgeDocumentSummary> listDocumentsByKeyword(
            String baseUrl, String apiKey, String datasetId, String keyword) {
            java.util.List<RetiredIndexBridgeDocumentSummary> matches = new java.util.ArrayList<>();
            for (Map.Entry<String, String> entry : documentsById.entrySet()) {
                if (entry.getValue() != null && entry.getValue().contains(keyword)) {
                    matches.add(new RetiredIndexBridgeDocumentSummary(entry.getKey(), entry.getValue()));
                }
            }
            return matches;
        }
        ...
        private void recordFragment(String filename) {
            ...
            // Mirror HttpRetiredIndexBridgeGateway#matchesContentHashFragment: the fragment is the token after the
