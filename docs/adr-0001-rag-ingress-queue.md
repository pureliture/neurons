# ADR-0001: 범용 RAG ingress queue와 RetiredIndexBridge target adapter
...
Local PC와 Mac mini에서 transcript/session/document ingest 요청이 burst 형태로 발생할 수 있다. 현재는 RetiredIndexBridge REST를 직접 호출하는 흐름이 있어 downstream parser backlog를 producer가 충분히 통제하지 못한다.

초기 target은 RetiredIndexBridge지만, 나중에 target RAG가 다른 솔루션으로 바뀔 수 있다. 따라서 서버를 `index-ingress-queue`로 잠그지 않고 `rag-ingress-queue`라는 범용 ingress queue로 설계한다.

`workspace-index-advisor`의 최신 구조에서는 RetiredIndexBridge를 장기기억 본문과 검색 대상이 모이는 LLM Wiki로 둔다. Mac mini의 기존 `transcript-capture`, `transcript-worker`, `transcript-packer`는 `session-compactor` 역할로 재분류한다. Ubuntu의 `memory-regeneration-runner`는 RetiredIndexBridge redacted corpus를 읽고 derived document를 다시 enqueue하는 batch producer다.

## Decision

`rag-ingress-queue`를 Java 25 + Spring Boot 기반의 별도 Docker Compose project로 만든다. Queue engine은 NATS JetStream을 사용한다. RetiredIndexBridge는 `RagTargetAdapter`의 첫 구현체인 `RetiredIndexBridgeAdapter`로 격리한다.
...
RetiredIndexBridge dataset partition은 target profile로 표현한다. 초기 RetiredIndexBridge profiles는 `index-transcript-memory`, `index-session-summary`, `index-task-summary`, `index-approved-memory-card`다. 단일 RetiredIndexBridge dataset에 metadata type만으로 모든 기억 데이터를 넣는 설계는 채택하지 않는다.
...
Pros: queue 기능을 직접 구현하지 않고, write path를 단일화하며, RetiredIndexBridge lock-in을 피한다.
...
Cons: 기존 RetiredIndexBridge Redis와 겹칠 위험이 있고, 현재 목적에는 runtime surface가 커질 수 있다.

## Consequences

- RetiredIndexBridge compose project를 수정하지 않아도 된다.
...
- RetiredIndexBridge-specific status와 IDs는 adapter-private metadata로 제한해야 한다.
- Mac-only private source access는 producer-side에 남고, Ubuntu worker는 provider transcript path를 읽지 않는다.
- RetiredIndexBridge document status table authorization pass 전 recall/promote는 계속 금지된다.
...
- 여기서 `Ledger`는 별도 장기기억 저장소가 아니라 RetiredIndexBridge document id/hash/state/provenance/authorization을 기록하는 external local 문서 상태표의 기존 코드 이름을 뜻한다.
- `rag-ingress-queue` worker는 Mac mini의 `ledger.py`/SQLite를 직접 갱신하지 않는다.
- RetiredIndexBridge target profile은 dataset role을 선택한다. Metadata `result_type`은 trace/filter 용도일 뿐 primary partition이 아니다.

## Done criteria

1. Direct RetiredIndexBridge write path가 producer에서 제거된다.
2. NATS JetStream stream/consumer가 redacted postcheck에 보인다.
3. RetiredIndexBridge delivery는 `RetiredIndexBridgeAdapter`를 통해서만 발생한다.
...
6. Derived memory documents도 direct RetiredIndexBridge write가 아니라 `/enqueue`를 통해 저장된다.
