...
- Replaced public `payload.kind=index_ready_document` with target-neutral `redacted_rag_ready_document`.
...
- Added live RetiredIndexBridge smoke as approval-gated and non-authorizing.
...
- Fake RetiredIndexBridge adapter/server tests for pressure states, upload/status mapping, and adapter-private raw ID handling.
...
- Live RetiredIndexBridge smoke only behind explicit approval, with evidence that `DONE -> INDEXED candidate` does not imply authorization.
