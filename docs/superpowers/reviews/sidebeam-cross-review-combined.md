...
- Target: rag-ingress-queue MVP (Java 25, Spring Boot 4.x, NATS JetStream, RAGFlow adapter)
- Reference Style: sidebeam-backend (Java 24, Spring Boot 3.5.4, GitLab external integration)
- Constraint: No live RAGFlow mutation. No Docker/Compose runtime mutation. No broad rewrites.

---

## Section 1: ACO Advisory Findings (from prior run)

### P1: StatusService RAGFlow-specific coupling
- StatusService directly depends on RagFlowGateway, RagFlowPressurePolicy, RAGFlow-specific config
...
### P2: RAGFlow adapter package separation
- RagFlow-specific classes mixed in generic target package
- Resolution: Move RAGFlow classes to target.ragflow sub-package
...
Adopt: Split target package into generic contract (target/) and adapter-private impl (target.ragflow/)
...
rag-ingress-queue: Controller directly assembles ResponseEntity with status codes; RagFlowDeliveryException is simple RuntimeException
...
rag-ingress-queue: target.{RagFlowGateway|HttpRagFlowGateway|RagFlowTargetAdapter}
           - No retry policy
           - Timeout hardcoded in adapter
           - HTTP client created inside adapter
           - @Value injection scattered

Adopt: RetryPolicy interface, global HTTP client config bean, @ConfigurationProperties for RAGFlow settings, external/internal DTO separation
...
- No Circuit Breaker for RAGFlow adapter
...
1. Package restructure: Move RAGFlow classes to target.ragflow/
...
8. Add RetryPolicy interface + simple retry for RAGFlow gateway
...
## Constraint Check
- No live RAGFlow mutation: All changes are code-only
