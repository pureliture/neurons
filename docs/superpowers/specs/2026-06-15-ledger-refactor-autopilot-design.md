...
- 데이터 층: **3 SQLite DB**(Ledger `agent_knowledge_ledger.db` 34테이블 / `RAGIngressStateDB` / `IngestStateStore`) + RAGFlow HTTP. (보고서의 '단일 SQLite'는 Ledger DB만 지칭 — scope 차이, 모순 아님. 보고서의 '33 테이블'은 stale; 실 코드·spec=34.) SQLite 종속 구문 광범위(근사: `INSERT OR IGNORE/REPLACE`×22, `ON CONFLICT`≈25, `PRAGMA`≈19, raw execute×142) — 정확 카운트는 S1 live grep으로 재산출(§2 수치 근사, freeze 금지).
...
envelope 구성:
1. **egress 2-tier 목적지 allowlist(OS-레벨, 단일 필수 기제)**: 강제는 **app 코드가 아니라 OS/프로세스 레벨** — autopilot이 spawn한 모든 PID(gate child=gradle/pytest/uv 포함)에 **호스트=macOS(Darwin arm64)라 Linux netns 없음·pf는 per-PID 불가 → per-UID 전용**: tier-A(Claude API)와 tier-B(gate child)를 **서로 다른 전용 UID**로 실행, pf 'user' 룰로 UID별 egress deny, `pf.conf`는 root/launchd 소유(agent UID 편집 불가). (대안: netns 실존 Linux VM/container 고정 — hard env 전제.) **허용 endpoint 리터럴 고정**: tier-B=추출 LLM `127.0.0.1:8930`만, tier-A=Claude API host:port. **`127.0.0.1:19380`(실 RAGFlow SSH 터널, `gate_measure.py:32`)는 양 tier DENY** + policy-proxy(`mcp_server.py:33`)·그 외 전부 deny. **이 egress 정책은 §5.0-5 root-of-trust pinned set 포함**(agent 수정 불가) — 안 그러면 gate(§5.2-ii)가 의존하는 방벽을 agent가 끄는 순환. self-test(c)가 **RagflowHttpClient 우회 raw socket이 `ragflow_url`에 fail**함을 단언. 이것이 비가역-삭제 blast의 *주* 방벽.
...
이 task 한정 명시 승인: **S0 envelope 사람 사인오프** → A1 코드 mutation 무인 + 통합 auto-merge; A2 delete merge 사람 승인 1회(코드 변경만 인가); main 무인 금지. A2 land 후에도 런타임 delete는 execute=False + standing forbidden-ops block 유지. standing pre-approval은 코드 mutation 미포함. **기존 llm-brain autopilot의 standing pre-approval(projection-write self-mint, `AUTOPILOT_PREAPPROVED=True`)은 이 루프에서 비활성 — 코드 변경 인가가 RAGFlow projection/delete *운영* 인가로 전이되지 않음.** CLAUDE.md guardrails는 §5.0 런타임으로 강제.
