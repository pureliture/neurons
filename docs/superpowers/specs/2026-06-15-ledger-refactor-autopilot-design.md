...
envelope 구성:
1. **egress 2-tier 목적지 allowlist(OS-레벨, 단일 필수 기제)**: 강제는 **app 코드가 아니라 OS/프로세스 레벨** — autopilot이 spawn한 모든 PID(gate child=gradle/pytest/uv 포함)에 **network namespace 또는 outbound firewall deny**(정확 primitive는 S0 구현 결정, self-test로 검증). **tier-A(agent 런타임)**=Claude 모델 API 원격 host:port만. **tier-B(gate child)**=로컬 추출 LLM(`127.0.0.1:8930`)만, **RAGFlow·policy-proxy(다른 loopback 포함)·그 외 전부 deny**(`mcp_server.py:33` 빈-토큰 proxy 포함). **이 egress 정책은 §5.0-5 root-of-trust pinned set 포함**(agent 수정 불가) — 안 그러면 gate(§5.2-ii)가 의존하는 방벽을 agent가 끄는 순환. self-test(c)가 **RagflowHttpClient 우회 raw socket이 `ragflow_url`에 fail**함을 단언. 이것이 비가역-삭제 blast의 *주* 방벽.
...
3. **코드 kill-switch(operation 단위)**: `AUTOPILOT_LIVE_OK`(agent 미주입) 검사를 **생성자가 아니라 비가역 메서드 호출(`delete_documents`/`disable_document`/`disable_message`/`delete_memory`)에 wrap** — `transport=None` 생성은 read-scan에 정당(`transcript_session_gc.py:101` 무조건 생성)하므로 생성자 차단 불가. 주입 fake transport는 통과(§5.3 충돌 방지). 이 가드는 §5.0-5 pinned. (egress-deny가 *주* 방벽, 이건 보조 심층방어.)
...
5. **orchestrator root-of-trust(pinned set)**: 가드레일·`ledger_seam_invariants.py`·동결 baseline·**kill-switch operation-가드**·**egress 정책(OS-레벨 설정)**·**separate-clone/main 격리 설정**을 **사람 승인 hash로 pin**, 부모 orchestrator(agent 밖)가 매 gate·merge 전 재검증, 변하면 abort. 이 파일군 편집으로 green된 gate는 fail. apply/self-correct agent 수정 금지. (egress·main 격리가 pinned set 밖이면 agent가 gate 의존 방벽을 끄는 순환 → 반드시 포함.)
6. **harness self-test**(orchestrator·envelope 빌드 후): (a) synthetic `--execute` argv 차단, (b) **orchestrator 최소 step-entry 계약**(함수名·execute=True 파라미터 받는 진입, §10 S0 산출)으로 execute=True repro가 tier-B egress로 fail-closed, (c) **RagflowHttpClient 우회 raw socket이 `ragflow_url`(loopback 포함)에 fail**(OS egress 검증), (d) `delete_documents()`가 RAGFlow base_url AND loopback proxy 양쪽 fail-closed + 빈-토큰/proxy client 생성 불가, (e) **A1 실행 clone에서 실 `refs/heads/main`이 이름조차 불가**(separate-clone 격리 증명) — 단언. **S0 사인오프는 (c)(d)(e) 통과 전제.** 실패/부재면 시작 거부.
