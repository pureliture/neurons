...
- source-to-candidate runtime readiness CLI can evaluate sanitized post-deploy evidence for MCP read/review tools, `brain_objects_query` route smokes, agent context `tool_hints`, required agent context sections, deployed identity, production-denial/safety smokes, and object authority production gate policy without network or mutation; required object-query routes include `authority_archive_separation`, `code_style_preference`, `temporal_work_recall`, and `deployment_runtime_truth`, agent-context tool hints must remain suggest-only/no-execute/no-production-mutation with safe targets and approval-board scope blockers, and production safety claims include source-to-candidate, approval-board, proposal-create, decision-commit, and object authority gate policy
...
- CLI object query route result: `37 passed, 1 warning`
...
- object/CLI/MCP adjacent result: `147 passed, 1 warning`
...
- worker regression evidence: `cd worker && uv run pytest -q`
- worker regression result: `1615 passed, 9 skipped, 1 warning`
- root regression evidence: `JAVA_HOME="$(/usr/libexec/java_home -v 25)" gradle test`
- root regression result: `BUILD SUCCESSFUL`
