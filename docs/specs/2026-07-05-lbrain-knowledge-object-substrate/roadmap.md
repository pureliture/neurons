...
- close the live authority overlay schema gap and rerun read-only configured-path smokes to confirm the overlay status becomes `available` without regressing route activation.
...
| P1 Production MCP Activation | `in_progress` | `PASS_WITH_GAPS`; baseline deployed/configured endpoint object-tool smoke, post-#111 image/source identity, and six-route object-pack proof pass, but authority overlay schema, deploy-script toolsList transport, source/review/readiness tool exposure, and broader production-readiness evidence gaps remain |
