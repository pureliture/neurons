...
- Post-#144 live rollout/capture: neurons PR #144 merged source commit `6d830e374527`, neurons-ops PR #21 updated production desired state at `4feafd41c656`, and Jenkins production deploy button #17 synced `neurons-oci-production` with sync `PASS`; the overall job result is `PASS_WITH_GAPS` / Jenkins `UNSTABLE` because the built-in postcheck recorded a `toolsList` gap. Argo reported `Synced` / `Healthy` at the ops revision, and the affected live runtime images rendered as `sha-6d830e374527`; `bulk-semantic-trigger` was scaled to desired replicas `0`, so no readiness claim is made for that workload. A separate read-only live MCP tools-list smoke returned 31 tools including `brain_objects_query`, `brain_context_resolve`, `brain_source_to_candidate_graph`, and `brain_source_to_candidate_runtime_readiness`. The deployed `mcp-http` image then collected a sanitized post-deploy MCP capture with `post_deploy_read_only_smoke`, `network_used=true`, `production_mutation_performed=false`, deployed identity containing the expected source commit, `agent_context_product_pack.v1`, and six route smokes. `source-to-candidate-runtime-readiness` replay is `PASS_WITH_GAPS` with `failed_claims=[]`, and activation progress replay is `PASS_WITH_GAPS` with no failed phases. Remaining gaps are explicit: P2 production corpus ingest evidence, P3 live graph/Qdrant projection join, P6 live session/project and multi-device rollup, P7 live preference/artifact memory, P8 verified runtime/permission/startup and shadow collection evidence, and P9 production consumer context/action-surface proof. The `llm-brain-tools` image can run `neuron-knowledge` but does not include the `mcp` client dependency needed for the capture collector, so post-deploy capture currently runs from the `mcp-http` image.
...
- PR #84부터 PR #95까지 `main`에 merge되었습니다. PR #97, PR #103, PR #105, PR #107, PR #109, PR #111, PR #113, PR #115, PR #119, PR #121, PR #122, PR #123, PR #124, PR #125, PR #126, PR #128, and PR #142 are post-#95 source/docs/evidence-gate follow-ups merged to `main`.
