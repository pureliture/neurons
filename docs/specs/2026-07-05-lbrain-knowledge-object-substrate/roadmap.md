...
- `brain_objects_query` local MCP read path now applies query-term coverage to non-empty `authority_archive_separation` requests: only relevant context-authority objects are returned, while no relevant match returns `objects=[]`, `context_authority_no_relevant_match`, and confidence `0`. Preference/style, HTML/visualization preference, temporal work recall, code-change-impact, and runtime-truth routes retain their route-specific packs or explicit gaps instead of falling back to `object_pack_route_not_implemented`
...
- FR6 route trace is now attached to branch-local `brain_objects_query` object packs with `route`, `route_source`, `selected_source_lanes` (empty when no relevant match), route `confidence`, `stop_reason`, and gap-derived `missing_evidence`; this explains routing decisions without treating local tests as deployed runtime proof
...
- activation guard는 graph trigger가 반환한 redacted `run_ref` (`sha256:` 뒤 12개 lowercase hexadecimal)를 graph-after와 final status의 exact run reference에 결속합니다. 같은 scope의 동시 실행이 더 최신이더라도 source run reference가 다르거나 누락되면 canary 전 단계에서 중단하며, stale run과 dead letter도 해당 exact run 기준으로 fail-closed 처리합니다.
- Graphiti entity edge는 `episodes`의 canonical source episode와 persisted Graphiti group 및 현재 graph scope를 exact-join해 provider provenance를 확인합니다. source provider가 비어 있거나 해소되지 않거나 scope가 다르거나, provider 값이 하나로 정규화되지 않거나, synthetic canary가 하나라도 포함되면 `GraphFact`를 반환하지 않습니다. provenance 해소 실패는 generic degraded detail로만 표시하고, Context와 runtime-readiness read lane은 malformed payload와 `source_providers`까지 방어적으로 제외합니다.
...
- legacy source document의 `source_locator_hash`가 **명시적으로 빈 값**인 경우에만, bounded provider/project 원본 scan에서 같은 document identity·content hash·session hash·정규화 가능한 observed interval·유효 locator hash를 모두 만족하는 후보가 정확히 하나일 때 locator와 temporal bounds를 하나의 CAS write로 복구합니다. 후보 없음·복수 후보·content/session/locator/revision drift·malformed nonempty locator는 추정하거나 재시도하지 않고 gap으로 남깁니다.
- 2026-08-03 corrective sub-checkpoint: live dry-run은 `target_document_limit=1`을 provider/project 전체 snapshot에 적용하면 실제 legacy target을 찾기 전에 막힌다는 것을 확인했습니다. targeted legacy repair는 단일 staged source에서 파생한 bounded `(session_id_hash, content_hash)` identity로 빈 locator legacy target만 조회하며, 전역 snapshot을 넓히지 않습니다. identity 상한, source file/entry·target document 상한, source parse, 후보 없음·복수 target·timeout은 모두 fail-closed입니다. execute 뒤 postcheck도 선택된 exact document만 재조회하며 provider/project-wide fallback을 허용하지 않습니다.
- 2026-08-10 source-side corrective checkpoint: 단일 staged Codex source가 256개를 넘는 candidate identity를 만들면 기존 candidate별 legacy lookup은 의도적으로 차단됩니다. 새 opt-in `--legacy-identity-targeted-intersection`은 source를 자르지 않고 전체 파싱한 뒤, `source_locator_hash=""` legacy target만 한 번의 bounded metadata snapshot으로 읽어 exact `(session_id_hash, content_hash)` 교집합이 source·target 각각 정확히 한 건일 때만 기존 CAS repair로 전달합니다. 후보 fanout 상한은 유지하며, normal locator target, parser 또는 excluded temporal candidate, 중복·malformed 교집합, snapshot/plan drift는 write 전에 fail-closed 됩니다. 이 checkpoint의 source proof는 focused 105개 및 전체 worker `3526 passed, 12 skipped`이며, image build·GitOps rollout·live dry-run·repair·Date A/B acceptance는 아직 별도 pending입니다.
