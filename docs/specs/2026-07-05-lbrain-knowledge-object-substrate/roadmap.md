...
- activation progress `product_evidence_checks` returns P6/P7/P9 `result=PASS`, P8 `result=PASS_WITH_GAPS` with `p8_runtime_evidence_unverified`, `p8_runtime_verified_evidence_missing`, and `p8_runtime_evidence_collection_plan_not_live_evidence`, and fails closed when required phase evidence is missing, mutation is claimed without evidence, or the runtime evidence collection plan claims network/live/mutation behavior
