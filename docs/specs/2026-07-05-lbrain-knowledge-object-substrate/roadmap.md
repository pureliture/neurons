...
- Current P7 work consumes a future runtime-marked `runtime_preference_artifact_memory` packet and blocks shadow/local-test or unsafe P7 packets from closing the gate; current configured LBrain read-path smokes show preference/style lanes are still empty, so this is P7 evidence-consumption preparation, not P7 production validation.
- Current P8 work consumes future supplied live runtime-authority packets only through separated `deployed_identity`, `permission_sensitive_audit`, and `evidence_provenance` claims. Identity-only evidence keeps the permission-audit gap, local replay keeps the live-evidence gap, and protected-value audit evidence fails closed. This is P8 evidence-consumption preparation, not deployed P8 production validation.
