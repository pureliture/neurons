...
- Production validation follow-up: `PASS_WITH_GAPS` overall; P1 MCP activation and P2 reference-corpus store gates are now `PASS` / `production_validated`, P3 deployed source-to-candidate review-loop and live graph/Qdrant projection-join evidence are validated, P4 deployed production authority gate policy/no-mutation, single-object bounded execution, rollback/demotion, approval-board-to-production integration, and full replacement-current prior/successor evidence are validated, and P6 live session/project/work-unit rollup evidence now validates. Local/safety gates, deployed/configured HTTP MCP smoke, post-#115 source/image identity, production deploy-button rollout, object authority schema ensure, source/review/readiness MCP tool exposure, production denied/no-mutation smokes, six-route `brain_objects_query` object-pack activation proof, bounded production Palantir reference-corpus ingest evidence, deployed P2-corpus-to-candidate-review-loop evidence, P3 live projection-join edge-count evidence, P4 deployed proposal/decision gate policy plus denial/no-mutation readiness proof, P4 one-shot synthetic `RepoDocument` bounded reject execution, P4 fresh synthetic accepted-current rollback-to-archive execution, P4 approval-board production promotion, P4 replacement-current demotion/promotion, P5 release-quality evaluator gates, and P6 live rollup gates all passed. PR #95 was merged on 2026-07-07 with merge commit `32f4fec`; PR #97, PR #103, PR #105, PR #107, PR #109, PR #111, PR #113, PR #115, PR #119, PR #121, PR #122, PR #123, PR #124, PR #125, PR #126, PR #128, PR #148, PR #150, PR #152, PR #174, PR #176, PR #178, PR #180, PR #182, and PR #184 are source/docs/evidence-gate follow-ups. Jenkins production deploy button #19 synced the neurons-ops #27 desired state for source `a6e2249381ab` after the canonical images were rebuilt. This is P1/P2/P3/P4/P5/P6 gate proof plus P7/P8/P9 source-side evidence preparation, not product-wide production readiness: P7/P8/P9 live runtime evidence remain gaps.
...
- P7 Preference, Style, And Artifact Memory: `PASS_WITH_GAPS` / `local_validated`; 현재 증거는 local/test only입니다. local/test artifact preference pack은 accepted/proposal lanes, profile objects, no-UI HTML artifact check, branch-local HTML/visualization preference object-query route, branch-local collector P7 preference/artifact memory packet shape, and post-deploy `html_visualization_preference` route-smoke contract를 검증합니다. Source-side gate는 이제 `current` preference만 `accepted_current` style-preference authority lane으로 승격하고, reference-only style/profile claim만 있는 경우 agent context section gap을 보존합니다. P7 runtime preference packet도 count-only evidence가 아니라 `accepted_current` lane과 `style_preference` context lane을 요구하며, post-deploy capture는 promotion blocker reason을 public-safe로 남깁니다. Live agent context pack 및 production authority promotion은 아직 gap입니다.
...
- P1, P2, and P6 are now `production_validated` for their bounded gates, while broader production-readiness evidence gaps remain in P7/P8/P9.
- P5 is `local_validated` and the release-quality evaluator gate is green, but product-wide production readiness remains blocked by P7/P8/P9 live evidence gaps.
...
P5 may be declared green only for the evaluator/release gate itself. Product-wide production readiness still requires P7/P8/P9 runtime, preference, and context evidence.
...
| P7 Preference, Style, And Artifact Memory | `local_validated` | `PASS_WITH_GAPS`; local/test artifact preference pack lanes, no-UI HTML artifact check, branch-local HTML/visualization preference route, post-deploy `html_visualization_preference` route-smoke contract, branch-local P7 runtime evidence packet validation, P5 product evidence gap surfacing, runtime-marked P7 post-deploy evidence consumption/fail-closed guards, PR #162 supplied-runtime preference evidence consumption gate, PR #182 source-side `current` preference to `accepted_current` style-preference authority lane gate, PR #184 source-side P7 packet accepted-current lane diagnostic guard, and PR #187 bounded `ArtifactPreference` production authority source gate pass. Current configured LBrain read path implements the routes but returns empty preference/style lanes, so deployed/live accepted preference authority, agent context preference section, and production authority promotion remain gaps |
| P8 Runtime Truth, Security, And Deployment Authority | `local_validated` | `PASS_WITH_GAPS`; local/test runtime authority policy, artifact identity join, private authority redaction, denial/no-mutation checks, sanitized source-to-candidate review-loop packet validation, sanitized bounded execution packet validation with required demotion step, sanitized activation-preview production-authority evidence gap closure, sanitized permission-sensitive audit packet validation, evidence provenance validation, current-session shadow evidence packet normalization, 6-route post-deploy route-smoke contract, branch-local collector packet generation, post-#142 deployed identity/provenance replay, one-step shadow readiness evaluation, PR #164/#166 separated P8 supplied-live identity/audit/provenance product-evidence consumption gate, and PR #174 GitOps desired-state-vs-live-deployed identity separation pass. Broader deployed/live production runtime authority, current-source permission audit collection, and production readiness remain gaps |
| P9 Agent Context Productization | `local_validated` | `PASS_WITH_GAPS`; local/test consumer compact packs, degraded/stale disclosure, surface policy, proposal-safe action hints, branch-local P9 startup/read-path packet validation, P5 product evidence gap surfacing, post-#142 live `tool_hints` capture replay, PR #168 degraded required-section actionability, PR #170 supplied-live P9 agent-context evidence consumption gate, PR #176 current-authority accepted-current product-section gate, PR #178 startup current-authority load gate, and PR #182 style-preference accepted-current section gate pass; deployed/live production startup read path, live accepted preference/style sections, and runtime enforcement remain gaps |
...
- PR #84부터 PR #95까지 `main`에 merge되었습니다. PR #97, PR #103, PR #105, PR #107, PR #109, PR #111, PR #113, PR #115, PR #119, PR #121, PR #122, PR #123, PR #124, PR #125, PR #126, PR #128, PR #142, PR #148, PR #150, PR #152, PR #162, PR #164, PR #166, PR #168, PR #170, PR #174, PR #176, PR #178, PR #180, PR #182, and PR #184 are post-#95 source/docs/evidence-gate follow-ups merged to `main`.
...
- PR #162 consumes a future runtime-marked `runtime_preference_artifact_memory` packet and blocks shadow/local-test or unsafe P7 packets from closing the gate; current configured LBrain read-path smokes show preference/style lanes are still empty, so this is P7 evidence-consumption preparation, not P7 production validation.
...
- PR #168 and PR #170 make degraded P9 agent-context gaps actionable and consume future supplied live P9 agent-context packets through `live.agent_context.tool_hints`, `live.agent_context.product_sections`, `live.agent_context.startup_read_path`, and `live.evidence.provenance`. PR #176 adds a source-side P9 product-section gate for `current_authority` accepted-current authority, and PR #178 adds a startup/read-path gate requiring `current_authority` to be loaded in startup section counts. PR #182 extends that source-side gate so `style_preference` objects must carry `accepted_current` when present; reference-only style/profile objects remain disclosed gaps. PR #184 aligns the P7 runtime preference packet with that same accepted-current lane requirement and keeps promotion blocker reasons public-safe. Complete supplied live evidence can close the P9 source-side phase gaps, but current production desired state and live startup/read-path proof have not been shown for the post-#178 source; this is P9 evidence-consumption preparation, not deployed P9 production validation.
