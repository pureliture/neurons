...
- focused result: `6 passed, 1 warning`
...
- adjacent regression result: `124 passed, 1 warning`
- worker regression evidence: `cd worker && uv run pytest -q`
- worker regression result: `1515 passed, 9 skipped, 1 warning`

Remaining gaps:

- P3 is not complete; only the reference corpus extraction preview, documentation cleanup strategy comparison, and runtime truth extraction preview slices are implemented
...
- session, PR/commit detail, style, and preference extractors are still planned
- evaluator coverage is limited to reference corpus, documentation cleanup, and PR merge/deploy truth golden query slices
...
| P3 Processing And Object Extraction Pipeline | `in_progress` | local/test reference corpus extraction preview, documentation cleanup strategy comparison, and runtime truth extraction preview pass; session/PR detail/style extractors, graph/search join, and broader evaluator coverage remain gaps |
