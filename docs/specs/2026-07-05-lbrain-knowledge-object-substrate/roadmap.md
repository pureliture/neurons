...
- Production validation follow-up: `PASS_WITH_GAPS`; local/safety gates와 baseline deployed/configured HTTP MCP smoke는 통과했지만 post-#107 current-source-main deployed MCP image identity는 미증명이며, production ledger/corpus/runtime mutation approval gate는 사전승인 상태입니다. PR #95는 2026-07-07에 merge commit `32f4fec`로 `main`에 merged 되었고, review-follow-up commit `0c70111`은 production gate/type validation 및 corpus ingest approval hash validation을 보강했습니다. PR #97는 2026-07-07에 merge commit `de62106`으로 post-merge roadmap/report source state를 정정한 source/docs-only patch입니다. PR #103, PR #105, and PR #107 subsequently merged source-side post-deploy capture aliasing, post-deploy capture product evidence gating, and P6/P7/P9 product evidence gap surfacing. 이 merge들은 source-delivery evidence이며 image rebuild, GitOps rollout, or live runtime proof가 아닙니다. 2026-07-08 KST post-#107 configured `mcp__lbrain.brain_objects_query` read-only six-route smoke still returned `object_pack_route_not_implemented` for all required routes. Branch-local runtime readiness는 이제 sanitized `production_authority_execution` evidence packet과 current-session shadow evidence packet을 검증합니다. 이 packet은 `live_evidence_provided=true`, `evidence_is_live=true`, `production_ready=false`, `production_mutation_performed=false`, `failed_claims=[]`로 평가되었으며, deployed/live production execution evidence는 아직 없습니다.
- Post-deploy capture handoff: source-side CLI/MCP now accept sanitized `post_deploy_capture` / `normalize_post_deploy_capture` inputs as operator-friendly aliases for the existing fail-closed runtime evidence normalizer/evaluator. P8 product evidence checks record that alias packet/report metadata and fail closed if post-deploy capture claims production readiness, skips network-use provenance, or reports production mutation. This improves handoff from ops runners but is not deploy, image identity, rollout, or live runtime proof by itself.
- P1 Production MCP Activation: `PASS_WITH_GAPS`; deployed/configured HTTP MCP는 baseline object-native tool exposure, read-only query, and production write denied/no-mutation smoke를 통과했지만 post-#107 current-source-main image identity는 증명되지 않았습니다. 현재 gap은 source-delivered `brain_source_to_candidate_graph`, `brain_candidate_review_edit`, `brain_approval_board_decide`, `brain_source_to_candidate_runtime_readiness` 및 six-route `brain_objects_query` object-pack proof의 live rollout/read-path proof 부재입니다.
...
Deployed HTTP MCP runtime activation and configured endpoint smoke passed for baseline object-native availability. That evidence is not a post-#107 current-source-main deployed image identity proof. The latest configured-endpoint smoke exposes object-native tools, returns a public-safe `brain_objects_query.v1` response, and denies production proposal/decision mutation without writes. Source-delivered route fixes still need deployed/live route-smoke proof.
...
- latest live production evidence recorded the Argo application tracking `main` and `Synced/Healthy`, but this desired-state/controller evidence does not by itself close MCP image source identity for post-#107 current `main`
...
- source repo `origin/main` contains PR #103 merge commit `30ad7d9`, PR #105 merge commit `d3a609e`, and PR #107 merge commit `fa42134`; these are source/docs/evidence-gate follow-ups, not deploy proof
- 2026-07-08 KST post-merge configured MCP read-only six-route smoke returned `object_pack_route_not_implemented` for `authority_archive_separation`, `code_style_preference`, `temporal_work_recall`, `code_change_impact`, `html_visualization_preference`, and `deployment_runtime_truth`, so source merge has not become deployed route proof
- 2026-07-08 KST after adding branch-local source matrix guards, configured `mcp__lbrain.brain_objects_query` read-only recheck still returned `object_pack_route_not_implemented` for all six required routes; source regression coverage improved, but deployed/live proof remains open
- redacted deployed MCP image identity was sufficient for baseline object-native tool availability, but post-#107 current-source-main deployed image identity remains `not_validated`
...
- latest configured route-smoke result: 2026-07-08 KST post-#107 read-only calls returned public-safe `brain_objects_query.v1` envelopes with `object_pack_route_not_implemented` gaps for all six required routes
- latest current Codex-session sanitized shadow evidence packet, with expected post-#107 source/image identity left unproven, evaluated to `PASS_WITH_GAPS` with `failed_claims=[]`, `live_evidence_provided=true`, `evidence_is_live=true`, `production_ready=false`, `production_mutation_performed=false`, evaluator `network_used=false`, evidence-side `evidence_collection_network_used=true`, and route-specific gaps for the required six-route smoke set, so deployed live proof remains incomplete and P1/P6/P7/P8/P9 stay `PASS_WITH_GAPS`, not `PASS`
...
- runtime readiness sanitized current-session shadow packet result: `status=PASS_WITH_GAPS`, `live_evidence_provided=true`, `failed_claims=[]`, `production_mutation_performed=false`, `network_used=false`, `evidence_collection_network_used=true`, `evidence_provenance.redaction_check=redacted_only`, and gaps include missing source-delivered source/review/readiness tools in live MCP, missing live agent context sections, `brain_objects_query_route_unimplemented:<route>` and `shadow_route_smoke_not_implemented:<route>` for the six required route-smoke routes, expected post-#107 source/image identity unverified, and bounded production authority execution unverified
...
- P5 six-route source matrix focused result: `18 passed, 1 warning`; this prevents source/CLI/MCP regression to `object_pack_route_not_implemented` but does not close the configured/deployed MCP route-smoke gap
...
| P1 Production MCP Activation | `in_progress` | `PASS_WITH_GAPS`; baseline deployed/configured endpoint object-tool smoke passed, but post-#107 current-source-main image identity and passing six-route object-pack proof remain gaps |
...
- PR #84 through PR #95 are merged into `main`; PR #97, PR #103, PR #105, and PR #107 are post-#95 source/docs/evidence-gate follow-ups merged into `main`.
...
- PR #103 added operator-facing post-deploy capture aliases for the existing sanitized shadow evidence normalizer/evaluator. PR #105 added product evidence gates that record post-deploy capture alias metadata without treating it as production readiness. PR #107 made P6/P7/P9 product evidence checks preserve live gaps as `PASS_WITH_GAPS` instead of plain `PASS`.
