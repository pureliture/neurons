...
- Graphiti entity edge는 `episodes`의 canonical source episode와 현재 graph scope를 exact-join해 provider provenance를 확인합니다. source provider가 비어 있거나 해소되지 않거나 scope가 다르거나, provider 값이 하나로 정규화되지 않거나, synthetic canary가 하나라도 포함되면 `GraphFact`를 반환하지 않습니다. provenance 해소 실패는 generic degraded detail로만 표시하고, Context와 runtime-readiness read lane은 malformed payload와 `source_providers`까지 방어적으로 제외합니다.
