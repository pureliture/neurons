...
M2의 첫 실제 repository extraction은 `CurationService.approve`를 `MemoryCurationRepository` port 뒤로 옮기는 것이다. 이후 Card 5 follow-up은 같은 private transaction seam을 사용해 `CurationService.supersede`의 partial-write 공백만 좁힌다. 기존 Ledger public API와 M1 private transaction seam은 유지하고, service layer가 Ledger write 세부에 직접 의존하는 면을 줄인다.
...
- 승인 상태: 사용자 사전 승인
- 핵심 요구사항:
  - `CurationService.approve`와 Card 5의 `CurationService.supersede` curation-owned write flow를 repository port로 이동한다.
...
1. 추천: default adapter repository
   - `MemoryCurationRepository` protocol을 `approve_candidate`와 `supersede_candidate` use-case port로 확장하고, `LedgerMemoryCurationRepository`가 `Ledger`와 `_LedgerTransaction`을 감싼다.
...
`CurationService`는 business flow를 유지하되, approval과 supersede write orchestration을 repository method로 위임한다. `LedgerMemoryCurationRepository`는 low-level Ledger proxy가 아니라 curation use-case adapter이며, SQL을 새로 작성하지 않고 existing Ledger/transaction-bound facade method를 호출한다. `MemoryCurationRepository`는 이번 milestone의 active use-case seam이지 stable public import contract가 아니다.
...
### Supersede candidate (Card 5 follow-up)

```text
CurationService.supersede(old_memory_id, candidate_id, approved_by, reason)
  -> ledger.get_memory_candidate(candidate_id)
  -> build_memory_card(candidate, supersedes=old_memory_id)
  -> repository.supersede_candidate(old_memory_id, candidate, card, approved_by, reason)
    -> Ledger._transaction()
      -> supersede_memory_card + old profile fact (if applicable)
      -> upsert_memory_card
      -> add_memory_card_evidence
      -> update_memory_candidate_state("approved")
      -> new profile fact (if applicable)
  -> return stored card
```
...
Card 5는 `supersede`의 public API와 return shape를 유지하면서 write orchestration만 transaction-bound repository path로 옮긴다. 이 흐름에서 예외가 나면 old card/authority와 profile facts를 포함한 모든 write가 rollback되어야 한다.
...
- 책임:
  ...
  - `supersede_candidate`의 old card demotion부터 new profile fact까지를 한 transaction으로 묶는다.
...
- 책임:
  ...
  - approval과 supersede write sequence는 repository에 위임한다.
  - `reject`, `disable`는 existing Ledger behavior를 유지한다.
...
- compatibility key인 `next_multi_write_candidate`는 `CurationService.supersede`의 Card 5 migration 완료와 private transaction operation을 기록한다.
...
- supersede 중 예외가 발생하면 old card/knowledge authority와 old profile fact를 포함한 prior writes도 같은 rollback으로 복구되어야 한다.
...
- service integration tests:
  ...
  - `CurationService.supersede`가 injected repository를 사용한다.
  - evidence 또는 마지막 new profile fact write 실패가 old/new supersede state 전체를 rollback한다.
...
- Card 5: Supersede transaction follow-up
  - Done: `CurationService.supersede`가 same private transaction 안에서 old card/authority, profile facts, new card/evidence/candidate writes를 수행한다.
  - Expected evidence: evidence failure와 final profile fact failure rollback fixtures.
...
- M2 first caller migration과 Card 5 supersede follow-up의 좁은 repository extraction에만 집중한다.
