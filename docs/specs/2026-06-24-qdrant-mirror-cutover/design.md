# RetiredIndexBridge→Qdrant Searchable-Mirror Cutover Design Spec

## Overview

RetiredIndexBridge의 vector retrieval(searchable mirror) 역할을 Qdrant로 무회귀·가역 단계로
...
`RetiredIndexBridgeAdapter`(이미 Qdrant adapter 존재) + `shadow_worker`(격리 dual-write
...
dual-write/parity 없이 한 번에 RetiredIndexBridge→Qdrant. **기각**: rollback 없음, 모든 human
...
```text
[Stage 1 code-only]
RagReadyDocument ─► QdrantDoclingMirrorAdapter (RetiredIndexBridgeAdapter)
...
[Stage 2+ live, gated]
shadow_worker ─(dual-write)─► RetiredIndexBridge(record) + Qdrant(shadow)
read-compare harness ─► recall@k + golden ─► evidence packet
recall mirror lane ─(Stage4 switch)─► Qdrant (RetiredIndexBridge fallback ON)
write cutover(Stage5) ─► no-fallback 소비자 이전 + Qdrant GC
RetiredIndexBridge 벡터 mirror disable(Stage6) ─► backup → retention → hard delete
```
...
4. (Stage 2) shadow가 RetiredIndexBridge와 Qdrant에 동시 기록하고 관찰만 한다.
...
8. (Stage 6) backup→승인→RetiredIndexBridge 벡터 mirror disable; hard delete는 retention 후.
...
| dual-write shadow | ingress payload | RetiredIndexBridge+Qdrant 기록, ShadowResult | shadow_worker 격리 스트림 |
| read-compare harness | query cohort | recall@k/mismatch/golden evidence | RetiredIndexBridge retrieve + Qdrant query + ledger |
...
- read-compare 회귀: 컷오버 중단, recall RetiredIndexBridge 유지.
...
- **M6 — dual-write shadow (live, gate)**: `shadow_worker` qdrant 분기로 격리
  dual-write(RetiredIndexBridge 무변경, 관찰). done=dual_write evidence(`total_count`,
  `target_profiles`) green, RetiredIndexBridge recall 무변경 확인.
...
- **M9 — write cutover + 소비자 이전 (live, gate)**: write 전환 + no-fallback 소비자
  (autopilot mining/index_read_sot/GC runners/backfill/supersede/native reconcile)
  이전 + Qdrant GC hard-delete chokepoint 연결. done=각 소비자 source 전환 test +
  GC dry-run/coverage/backup/rollback evidence.
- **M10 — RetiredIndexBridge 벡터 mirror disable (live, gate, 비가역 마지막)**: backup→승인→
...
- **M8 배선 항목(아키텍처 리뷰 후속)**: (1) `brain_query._combine_query_lanes`의 기존
  RetiredIndexBridge mirror-merge를 `join_mirror_hits_to_authority` 단일 seam으로 수렴시켜 두
...
- recall@k parity 정량 임계(M7에서 RetiredIndexBridge 기준선 측정 후 고정).
- 단일 collection vs privacy 분리(접근정책 입력 시; 기본 단일).
- searchable-mirror gate ↔ M9 closure-chain schema 연결 지점(M10 착수 시).
- delete capability를 `retired_index_bridge.py` neutral 경계로 승격할지(M9에서 판단;
