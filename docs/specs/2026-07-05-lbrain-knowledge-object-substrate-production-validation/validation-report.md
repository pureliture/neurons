...
- result: returned `source_to_candidate_runtime_evidence.v1` with collector metadata, `source_to_candidate_review_loop_evidence.v1`, `session_project_rollup_runtime_evidence.v1`, `collector_packet_not_live_evidence`, `collection_mode=local_test_replay`, `network_used=false`, and `production_mutation_performed=false`; evaluating that packet returns `source_to_candidate_runtime_readiness.v1`, `status=PASS_WITH_GAPS`, `failed_claims=[]`, `live.brain_objects_query.route_smokes.status=validated`, `live.source_to_candidate.review_loop.status=validated`, and `live.session_project.rollup.status=validated` for local_test shadow evidence.
- interpretation: this turns branch-local read-only route smokes plus local_test source-to-candidate review-loop and P6 session/project/work-unit rollup smokes into reusable evaluator input and proves the collector packet shape. It does not prove deployed source/image identity, live MCP tool loading, deployed source-to-candidate wiring, live multi-device runtime rollup, or production readiness.
...
- P8 collector packet generation is branch-local read-only evidence preparation only. It can create a public-safe `source_to_candidate_runtime_evidence.v1` packet from local route smokes plus local_test source-to-candidate review-loop and P6 session/project/work-unit rollup smokes and validate that shape, but it remains `collector_packet_not_live_evidence` until the same packet shape is populated from deployed/configured live evidence with source/image identity proof.
