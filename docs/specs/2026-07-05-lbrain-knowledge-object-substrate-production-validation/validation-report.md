...
PR #73 및 ops deploy-button merge 이후의 이전 recheck는 P1 object-tool availability를 뒷받침하는 historical evidence입니다. PR #95는 2026-07-07에 merge commit `32f4fec`로 `main`에 merged 되었지만, 현재 recheck 상태는 `PASS_WITH_GAPS`로 유지됩니다: current Codex-session `mcp__lbrain` read path는 `brain_objects_query`를 호출할 수 있지만 retry-normalized required 6-route smoke가 모두 `object_pack_route_not_implemented`를 반환했습니다. Post-merge read-only smoke for `deployment_runtime_truth` also returned `object_pack_route_not_implemented`. Post-#95 live MCP image identity and route behavior remain unproven.
...
- evidence: configured Codex namespace can now run `brain_objects_query`, but the post-merge route needed for deployment/runtime truth returned `object_pack_route_not_implemented`, and the source-delivered #95 source/review/readiness tools are not loaded in this session.
