...
- P8 collector packet generation is branch-local read-only evidence preparation only. It can create a public-safe `source_to_candidate_runtime_evidence.v1` packet from local route smokes plus a local_test source-to-candidate review-loop smoke and validate that shape, but it remains `collector_packet_not_live_evidence` until the same packet shape is populated from deployed/configured live evidence with source/image identity proof.
