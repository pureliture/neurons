...
- result: returned `source_to_candidate_runtime_evidence.v1` with collector metadata, `collector_packet_not_live_evidence`, `collection_mode=local_test_replay`, `network_used=false`, and `production_mutation_performed=false`; evaluating that packet returns `source_to_candidate_runtime_readiness.v1`, `status=PASS_WITH_GAPS`, `failed_claims=[]`, and `live.brain_objects_query.route_smokes.status=validated`.
- interpretation: this turns branch-local read-only route smokes into reusable evaluator input and proves the collector packet shape. It does not prove deployed source/image identity, live MCP tool loading, or production readiness.
...
- P8 collector packet generation is branch-local read-only evidence preparation only. It can create a public-safe `source_to_candidate_runtime_evidence.v1` packet from local route smokes and validate the route-smoke shape, but it remains `collector_packet_not_live_evidence` until the same packet shape is populated from deployed/configured live evidence with source/image identity proof.
