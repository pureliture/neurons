...
- base skeleton 단독 apply 금지: PVC `storage` 같은 schema 필수 필드는 공개 산출물에 실제 크기를
  넣지 않으려고 비워 둔다. base는 overlay kustomize build 후에만 apply하며, overlay 없이 apply하면
  필수 필드 누락으로 fail-closed로 막힌다. (HPA `maxReplicas`는 schema 충족용 단자리 안전 기본값만 가짐.)
