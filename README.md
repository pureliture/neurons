...
| **Session/project-memory** | session-memory build + RAGFlow recall projection | neurons (worker) | 🟢 live (project-memory는 deferred view) |
...
live queue consume(별도 compose project) · `INGRESS_DELIVERY_BACKEND`(ragflow|couchdb) ·
...
> (single-consumer invariant). 배달 backend는 `INGRESS_DELIVERY_BACKEND`로 `ragflow`(코드 기본)
...
정의하며 기존 RAGFlow stack·volume은 수정하지 않는다.
...
|---|---|
| `ragflow-transcript-memory` | `transcript-memory` *(retired projection target)* |
| `ragflow-session-memory` | `session-memory` *(live recall surface)* |
...
> fail-closed로 거부한다(`assert_ragflow_target_allowed`). raw dataset ID는 generic API 출력·log·docs
...
> ledger는 각 knowledge item의 **단일 진실원**이다. RAGFlow는 검색 가능한 mirror일 뿐, 권위가 아니다.
...
> 파생되어 RAGFlow recall로 projection된다.
...
이를 RAGFlow `session-memory` dataset으로 projection하며,
...
> 또한 live shadow_worker의 sink인 `CouchDBIndexBackendAdapter`와 drain 경로의
> `CouchDBDeliveryBackend`는 서로 다른 대상이다.

- **recall** — `brain.query`(`brain_id=/project/<project>`)가 RAGFlow hit을 ledger active-set과
  join한다. `status=active` card만 살아남고, RAGFlow는 mirror일 뿐 권위가 아니다.
...
> RAGFlow native-memory와 graph는 **second-class mirror**일 뿐이다(`winner=local_ledger`).
...
- augmenting mirror는 gate된다 — RAGFlow mirror는 `--dataset-id`, native-memory recall은
  `--native-memory-id` / `RAGFLOW_NATIVE_MEMORY_ID`, graph는 enable 시에만.
...
<tr><td>credential</td><td><code>RAGFLOW_API_KEY</code> (단일 토큰)</td></tr>
...
> Docker daemon/Compose 런타임, live RAGFlow, live PG, live GC 검증은 **각각 별도 gate**다.
...
`SM_STATE_DIR` + `RAGFLOW_API_KEY` + `NEURON_LEDGER_PG_DSN`를 요구.
**RAGFlow와 ledger-PG는 out-of-repo**(loopback / `host.docker.internal`)이며
...
> 🔐 **단일 토큰 규칙.** credential은 `RAGFLOW_API_KEY` **하나만** 쓴다.
> `RAGFLOW_WRITE_TOKEN` / `RAGFLOW_READ_TOKEN`을 새로 만들지 않는다. write/GC/migration
...
6. **ledger가 상태 권위다** — `status`(진행)와 `authorization_status`(recall 가능)를 분리하고,
   RAGFlow/graph는 mirror일 뿐 권위가 아니다.
