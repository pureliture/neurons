# RetiredIndexBridge → Qdrant searchable-mirror 교체 readiness

이 디렉터리는 RetiredIndexBridge의 **vector/search mirror** 역할을 Qdrant로 교체하기 위한
...
- `docs/specs/2026-06-21-qdrant-docling-searchable-mirror/{requirements,design,milestones}.md`
  ...
  RetiredIndexBridgeAdapter 호환, evidence packet gate, production NO-GO", embedding
  model은 명시적 미결정.
- `specs/recall-cutover/{requirements,design}.md` — recall을 RetiredIndexBridge
  `session-memory`로 컷오버하고 transcript-memory를 은퇴(RC1–RC6 done). 단 그 이후
  recall은 **ledger-first**로 옮겨졌다(권위 read model = ledger; RetiredIndexBridge
  ...
  §0의 검증 항목. (recall-cutover의 "RetiredIndexBridge session-memory recall"은 그 시점
...
| 포함 | 제외 |
| --- | --- |
| RetiredIndexBridge read/write 의존성 audit | live RetiredIndexBridge write/delete/disable |
...
- RetiredIndexBridge는 **searchable runtime mirror + vector retrieval**이고 source of truth가
...
- backend-neutral 경계는 `rag_ingress/retired_index_bridge.py`의 `RetiredIndexBridgeAdapter`
  protocol이다. 현재 구현 adapter는 `RetiredIndexBridgeRetiredIndexBridgeAdapter`,
  `CouchDBRetiredIndexBridgeAdapter`, 그리고 PoC인 `QdrantDoclingMirrorAdapter` 셋이다.
...
1. [`01-index-dependency-audit.md`](01-index-dependency-audit.md) — RetiredIndexBridge
   read/write 의존성 전수 audit과 "RetiredIndexBridge를 끄면 무엇이 깨지는가" 분류.
...
4. [`04-cutover-gates-and-rollback.md`](04-cutover-gates-and-rollback.md) —
   ...
   "RetiredIndexBridge를 언제 안전하게 끌 수 있는가" 단계별 gate ladder.
...
- `RETIRED_INDEX_BRIDGE_API_KEY` 하나만 사용. 새 token env 도입 없음.
