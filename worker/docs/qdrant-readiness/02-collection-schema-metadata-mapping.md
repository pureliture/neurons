...
RetiredIndexBridge는 dataset 단위로 격리되고 parse 시점에 server-side로 chunk·embed한다.
...
Qdrant가 대체하는 것은 RetiredIndexBridge의 **vector retrieval(searchable mirror)**이다. 따라서
적재 대상 = **현재 recall surface가 RetiredIndexBridge retrieval로 검색하는 dataset**이다.

- `recall-cutover/requirements.md` Q1(완료, RC1–RC6 done): **컷오버 후 normal
  recall의 권위 surface는 RetiredIndexBridge `session-memory`다.** transcript-memory는 은퇴
  ·삭제됐고, recall 진입점은 session-memory로 재배선됐다(RC3 done).
- 따라서 **session-memory가 Qdrant 미러의 1차 적재 대상이다.** session recap도
  session-memory dataset(`index-session-memory` target_profile)으로 흘러 같은
  surface에 들어간다.
...
- **현재 코드 상태(검증됨)**: `mcp_server.KnowledgeSearchService.brain_query`의
  권위 read model은 **ledger**(`LegacyLedgerBrainReadModel`)이고, RetiredIndexBridge
  `retrieve` lane은 `dataset_ids`가 주어질 때만 켜지는 **옵션**이다
  (`index_search = self._brain_query_index_search if self.dataset_ids else None`).
  `cli.py`의 `--state-db-recall` / `--retired-index-bridge-direct-recall` 플래그는 현재
  **파싱만 하고 버려지는 no-op**이라 recall 경로가 더는 그걸로 분기하지 않는다.
- **결론(이 readiness의 운용 기본값)**: recall은 **ledger-first**이며 RetiredIndexBridge
  ...
  cutover(RC done)에서 session-memory가 RetiredIndexBridge retrieval surface였던 것은 그
  시점 상태이고, 이후 ledger-first로 옮겨졌다.
- **유일하게 남은 런타임 확인**: 라이브 recall mcp-stdio가 `--dataset-id` 없이
  떠 있으면 RetiredIndexBridge 벡터 recall은 사실상 미사용이다. 이 launch 인자는 repo 밖
...
| 항목 | 결정 | 근거 |
| --- | --- | --- |
| collection 수 | **단일 collection + payload filter**를 v1 기본으로, `privacy_class` 격리가 정책상 필요하면 privacy 단위 분리를 옵션으로 | RetiredIndexBridge는 14개 logical role(`dataset_contract.py`)을 dataset로 분리하지만, Qdrant payload index로 동등 필터 가능. multi-tenant/접근정책 격리가 hard 요구면 collection 분리 |
...
| logical→collection 매핑 authority | ledger에 `qdrant_collections` 레지스트리 신설(아래 4절) | 현재 index_datasets만 있음 |
...
- 동일 natural key 재upsert는 같은 point로 떨어져 **암묵적 dedup/overwrite**가 된다.
  RetiredIndexBridge가 `update_metadata`로 meta_fields에 content_hash+idempotency_key를 심어
...
### 3.2 RetiredIndexBridge filter parity 요구

`index_client.retrieve`/`transcript_memory_records_from_retired_index_bridge` 등이 실제로 거는
...
| RetiredIndexBridge filter key | Qdrant payload 위치(현재) | production 권고 |
| --- | --- | --- |
...
> 명명 일관성: RetiredIndexBridge는 `result_type`과 `type`을 혼용한다
...
RetiredIndexBridge는 ledger `index_targets`(컬럼: `logical_name`, `dataset_id`,
...
authoritative하지 않다**. RetiredIndexBridge 경로가 `ledger.authorize_document`로 모든 후보를
