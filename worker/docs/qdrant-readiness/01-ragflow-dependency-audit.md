# 01. RAGFlow read/write 의존성 audit

RAGFlow를 searchable mirror에서 떼어내기 전에, 어떤 코드가 RAGFlow에 read/write로
묶여 있는지와 RAGFlow를 끄면 각 경로가 어떻게 동작하는지를 전수 정리한다. 모든
client 호출은 `agent_knowledge/ragflow_client.py`의 `RagflowHttpClient`를 거친다.

핵심 분류 축: **fallback 보유 여부**. recall 표면의 일부는 ledger/CouchDB가
authority라 RAGFlow가 없어도 degrade만 하지만, 일부 경로는 RAGFlow가 사실상 유일
source라 RAGFlow를 끄면 빈 결과/정지가 된다. 이 "no-fallback" 경로들이 RAGFlow
disable의 진짜 blocker다.

## A. Read plane

### A-1. RAGFlow client read surface (`ragflow_client.py`)
...
| `search_messages` | RAGFlow Memory semantic search | native-memory recall, superseded message reconcile |

### A-2. recall/read 소비 표면

| 파일 / symbol | 역할 | RAGFlow off 시 동작 | fallback |
| --- | --- | --- | --- |
| `llm_brain_core/document_bridge.py` · `RagFlowDocumentBridge.search_documents` | brain.query document-bridge lane | status=`unavailable` 반환 | ✅ lane만 비고 ledger 결과 유효 |
...
| `session_memory/memory_regeneration.py` · `RagflowRetrievalTranscriptMemorySource` | RAGFlow 후보 + ledger authority hybrid | 후보 set 비어 결과 0 | ✅ `LedgerTranscriptMemorySource` |
...
| `session_memory/memory_regeneration.py` · `RagflowTranscriptMemorySource` (`ragflow_read_sot` mode) | Mac-ledger-free session-memory build source | source chunk 0 → 문서 미생성 | ❌ **no-fallback** |
...
| `session_memory/memory_regeneration.py` · `mark_ragflow_done_*` | queued 문서 status reconcile | queued가 영구히 안 풀림 | ❌ ledger 상태 stuck |
...
| `session_memory/ragflow_projection.py` · `_find_existing_projection_document` | upload 전 idempotency 확인 | 빈값 → 중복 upload 가능 | ⚠️ 중복 위험 |
| `couchdb_source/ragflow_fallback.py` · `RagflowReader` | CouchDB 원본 소실 세션 복구 | 복구 불가 | (의도된 recovery 전용) |
| `rag_ingress/index_backend.py` · `RAGFlowIndexBackendAdapter.find_by_natural_key`/`document_status*` | ingest dedup·parse 상태 | 예외 → 재시도(중복 위험) | ⚠️ |
| `src/.../ragflow/HttpRagFlowGateway.java` · `findByContentHash`/`listDocumentsByKeyword` | Java ingest dedup | 예외 → delivery 재시도 | ⚠️ |

## B. Write plane

RAGFlow write는 5종뿐이다: `upload_document` + `update_metadata` + `request_parse`
...
| --- | --- | --- |
| `rag_ingress/index_backend.py` · `RAGFlowIndexBackendAdapter.submit_document` | upload+meta+parse | ✅ `rag-ingress-worker`(shadow_worker, `SHADOW_DELIVER=1`, Ubuntu 상주 NATS consumer) |
| `rag_ingress/delivery_backend.py` · `RagflowDeliveryBackend.submit` | adapter 위임 | operator-gated(`drain-deliveries`); CLI gate가 RAGFlow live 차단 |
| `rag_ingress/delivery_drain.py` · `drain_pending_deliveries` | delivery 실행 | CLI-only, RAGFlow live path는 `state_cli`에서 차단 |
...
| `session_memory/ragflow_projection.py` · `upsert_memory_card` | upload+meta+parse | ✅ autopilot `--allow-write`(standing pre-approval) |
...
| `couchdb_source/ragflow_projector.py` · `project` | upload+meta+parse | neuron-knowledge couchdb build |
| `rag_ingress/file_ingest.py` · `submit_file` | upload+parse | 미연결(라이브러리 함수) |
| `rag_ingress/shadow_worker.py` · `main` | backend 선택 gate | `INGRESS_DELIVERY_BACKEND`=ragflow/couchdb 분기 |

## C. RAGFlow disable의 진짜 blocker (요약)

RAGFlow를 끄려면 아래 **no-fallback 소비자**가 먼저 다른 source(CouchDB / Qdrant /
ledger)로 이전되거나 은퇴해야 한다:

1. **autopilot live mining** (`mine_live_candidates`) — session/transcript-memory를
   RAGFlow에서만 읽는다.
2. **`ragflow_read_sot` session-memory build** — RagflowTranscriptMemorySource.
3. **GC runners 3종** (session/transcript-session/transcript-volume) — 후보 열거를
   RAGFlow에 의존(끄면 삭제는 안전하게 멈추지만 GC 진행이 전면 정지).
...
7. **status reconcilers** (`mark_ragflow_done_*`) — queued 영구 stuck.
...
read나 CouchDB read로의 **개별 repoint가 필요**하며, 그 전까지 RAGFlow read는 끌 수
