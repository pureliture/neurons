...
| | RAGFlow | Qdrant |
| --- | --- | --- |
| embedding 주체 | **RAGFlow server**가 `request_parse` 시점에 chunk+embed | **worker**가 upsert 전에 vector를 직접 생성 |
| 모델 위치 | RAGFlow 내부(dataset의 embedding_model) | `EmbeddingProvider` seam(worker 소유) |
| worker가 보내는 것 | raw markdown 본문 | `[float]` vector + payload |

즉 Qdrant 교체는 단순 store 교체가 아니라 **embedding ownership을 RAGFlow에서
...
- Qdrant `EmbeddingProvider`는 **이 동일 endpoint를 재사용**한다 — 같은
  ...
  있는 OpenAI-compatible reranker를 mirror 후보 재정렬에 쓴다(RAGFlow `rerank_id`
  자리 대체).
...
RAGFlow는 upload → update_metadata → request_parse → (server embed) → poll DONE의
...
- point_id가 natural key이므로 upload+metadata 2단계가 1단계로 합쳐진다
  (RAGFlow는 meta_fields를 별도 PATCH했지만 Qdrant는 payload에 원자적으로 포함).
- `submit_document`은 즉시 `IndexStatus.INDEXED`를 반환한다 — RAGFlow의 비동기
  parse(UNSTART→RUNNING→DONE)와 달리 upsert는 동기 완료다. 따라서
  `mark_ragflow_done_*` 류 status reconciler는 Qdrant 경로에서 불필요해진다(문서
...
- RAGFlow에는 update-in-place가 **없다**(본문 변경=새 문서 upload).
...
- backend-neutral `IndexBackendAdapter` protocol에는 **delete가 없다**. RAGFlow
  document delete는 adapter를 우회해 `gc_safety_auditor.hard_delete_documents`
  단일 chokepoint(→ `ragflow.delete_documents`)를 지난다.
- RAGFlow에는 `disable_document`(soft, enable=0)가 있어 hard delete 전 가시성
...
- `MirrorDeletionCapable` protocol — `IndexBackendAdapter` 위에 얹는 **optional**
  delete 능력. neutral 경계(`index_backend.py`)는 건드리지 않고
  `qdrant_docling_mirror.py`에 두었다(boundary regression 최소화). 다음 단계에서
  index_backend로 승격할지는 별도 결정.
