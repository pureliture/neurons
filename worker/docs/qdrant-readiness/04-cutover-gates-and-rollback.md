...
이 문서는 "RetiredIndexBridge를 **언제 안전하게 끌 수 있는가**"를 단계별 gate로 고정한다. 각
...
- **recall@k overlap**: 동일 query 집합에 대해 RetiredIndexBridge `retrieve` top-k와 Qdrant
...
## 2. cutover ladder (RetiredIndexBridge OFF까지)
...
각 Stage는 **이전 Stage의 evidence가 green일 때만** 진입한다. RetiredIndexBridge는 마지막
Stage(RetiredIndexBridge 벡터 미러 disable) 이전까지 항상 ON이며 authority/fallback으로 남는다.

### Stage 0 — code-only readiness (현재, 이 작업으로 충족)

- Qdrant adapter + delete seam + 재사용 fake + contract test 존재, worker test green.
- live wiring/routing/env 변경 없음. RetiredIndexBridge 100% ON.
...
### Stage 2 — dual-write shadow (RetiredIndexBridge가 record-of-authority)

- `shadow_worker` 패턴으로 Qdrant를 **두 번째 sink**로 추가(격리 스트림, RetiredIndexBridge
  delivery 무변경). `INGRESS_DELIVERY_BACKEND`에 qdrant 분기 추가 가능하나
  **live recall은 여전히 RetiredIndexBridge에서만** 제공.
- dual_write evidence(`total_count`, `target_profiles`) 수집.
- exit 증거: `dual_write` 섹션 green, RetiredIndexBridge recall 무변경 확인.

### Stage 3 — shadow-read parity (관찰만)

- read_compare harness로 동일 query에 대해 RetiredIndexBridge vs Qdrant 결과를 ledger-join
...
### Stage 4 — read cutover (mirror lane만, RetiredIndexBridge ON 유지)
...
- recall의 **mirror/vector lane**만 Qdrant로 전환: brain.query archive/evidence
  ...
  source. ledger/CouchDB authority join은 유지. RetiredIndexBridge는 **fallback**으로 ON.
...
- RetiredIndexBridge write(session-memory sync, ingress delivery, card projection)를 Qdrant/
  CouchDB로 전환. Qdrant write path + delete chokepoint(문서 03 §4.4) 연결.
- **문서 01 §C의 no-fallback read 소비자 전부 이전/은퇴**: autopilot mining,
  `index_read_sot` build, GC runners 3종, backfill seed, native-memory reconcile,
...
### Stage 6 — RetiredIndexBridge disable (최종, 되돌릴 수 있게)
...
- searchable-mirror gate evidence(`...searchable_mirror_gate_evidence.v1`)를 M9
  ...
  **cross-reference로 추가**(현재 미연결). closure approval record의 apply_sequence에
  "index_search_mirror_disable" 단계를 추가하고, unredacted operator binding +
  rollback bytes 보존을 요구.
- disable은 즉시 hard delete가 아니라 **retention/stability window** 동안 RetiredIndexBridge
...
| 시점 | rollback 방법 |
| --- | --- |
| Stage 2–3 | Qdrant sink/관찰 비활성화. RetiredIndexBridge가 계속 authority라 사용자 영향 0. |
| Stage 4 | recall mirror lane을 RetiredIndexBridge로 즉시 환원(switch plan rollback manifest의 bytes/digest로 config 복원). |
| Stage 5 | write를 RetiredIndexBridge로 환원. Qdrant는 mirror라 데이터 재-ingest로 복구(본문은 ledger/CouchDB 보유). |
| Stage 6 | RetiredIndexBridge는 disable만 한 상태(데이터 보존)이므로 re-enable로 복귀. hard delete는 retention window 이후 별도 승인. |

rollback 불가 지점은 **RetiredIndexBridge 데이터의 hard delete**뿐이다 → 이는 disable·soak·
승인·backup 이후로만 분리한다(GC 규약).

## 4. "RetiredIndexBridge를 끌 수 있는가" 판정 체크리스트 (Stage 6 진입 gate)
...
위 9개가 모두 충족되기 전에는 RetiredIndexBridge searchable mirror를 끄지 않는다.
