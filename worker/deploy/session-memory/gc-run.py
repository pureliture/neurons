...
실행(--execute). RAGFlow base는 env. ledger는 NEURON_LEDGER_PG_DSN으로 PG. 패키지 설치됨.
...
token = os.environ["RAGFLOW_API_KEY"]
r = RagflowHttpClient(base_url=base, bearer_token=token, request_timeout_seconds=45)
...
def approve(path, op, ds, argv):
    pathlib.Path(path).write_text(json.dumps({
        ...
        "timeout_seconds": 1200, "target": {"dataset_id": ds, "ragflow_base_url": base},
        "command": {"argv": argv},
    }))
...
sds = _resolve_dataset_id(ragflow=r, dataset_name="session-memory")
tds = _resolve_dataset_id(ragflow=r, dataset_name="transcript-memory")
sa = ["--ledger", "state/neuron-ledger.sqlite", "--dataset-id", sds, "--ragflow-url", base,
...
va = ["--ledger", "state/neuron-ledger.sqlite", "--transcript-dataset-id", tds, "--ragflow-url", base,
...
ta = ["--transcript-dataset-id", tds, "--session-memory-dataset-id", sds, "--ragflow-url", base,
