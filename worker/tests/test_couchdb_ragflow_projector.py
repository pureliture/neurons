"""Unit tests for RagflowSessionMemoryProjector.

Uses a FakeRagflowClient injected via monkeypatching so no live RAGFlow calls
are made. Verifies:
...
- the RAGFlow document id (ref string) is returned
"""
...
from agent_knowledge.couchdb_source.ragflow_projector import (
...
class FakeRagflowClient:
    """Simulates RagflowHttpClient with minimal method stubs.
    ...
    def __init__(self, *, dataset_name: str = "session-memory", dataset_id: str = "ds-001") -> None:
        ...
        self._next_doc_id = "ragflow-doc-001"
...
def _make_projector(fake_client: FakeRagflowClient, dataset_name: str = "session-memory") -> RagflowSessionMemoryProjector:
    """Create a projector with the fake client injected (bypasses __init__ HTTP construction)."""
    projector = object.__new__(RagflowSessionMemoryProjector)
    projector._ragflow = fake_client
...
class TestTranscriptMemoryRejected:
    """assert_ragflow_target_allowed must reject retired profile before any I/O."""
    ...
    def test_retired_profile_raises_ownership_violation(self) -> None:
        ...
        doc = _projection_doc(
            ...
            content_hash=_chash(),
            target_profile=RETIRED_RAGFLOW_PROFILE,
        )
        with pytest.raises(OwnershipViolation, match="transcript-memory"):
            projector.project(target_profile=RETIRED_RAGFLOW_PROFILE, document=doc)
    ...
    def test_no_ragflow_calls_when_profile_rejected(self) -> None:
        fake = FakeRagflowClient()
        projector = _make_projector(fake)
        doc = _projection_doc(session_id_hash=_sid(), content_hash=_chash(), target_profile=RETIRED_RAGFLOW_PROFILE)
        try:
            projector.project(target_profile=RETIRED_RAGFLOW_PROFILE, document=doc)
...
class TestDatasetResolutionByName:
    ...
    def test_list_datasets_called_with_correct_name(self) -> None:
        fake = FakeRagflowClient(dataset_name="session-memory", dataset_id="ds-abc")
        projector = _make_projector(fake, dataset_name="session-memory")
        doc = _projection_doc(session_id_hash=_sid(), content_hash=_chash())

        projector.project(target_profile=RAGFLOW_RECALL_PROFILE, document=doc)
    ...
    def test_no_matching_dataset_raises(self) -> None:
        class _EmptyClient(FakeRagflowClient):
            def list_datasets(self, *, name="", dataset_id="", include_parsing_status=False):
                self.list_datasets_calls.append({"name": name})
                return []
...
class TestWriteSequence:
    ...
    def test_write_uses_correct_dataset_id(self) -> None:
        fake = FakeRagflowClient(dataset_id="ds-correct")
        projector = _make_projector(fake)
        doc = _projection_doc(session_id_hash=_sid(), content_hash=_chash())
        projector.project(target_profile=RAGFLOW_RECALL_PROFILE, document=doc)
    ...
    def test_request_parse_receives_document_id(self) -> None:
        ...
        projector = _make_projector(fake)
        doc = _projection_doc(session_id_hash=_sid(), content_hash=_chash())
        ref = projector.project(target_profile=RAGFLOW_RECALL_PROFILE, document=doc)
...
class TestIdempotency:
    ...
    def test_second_call_same_content_skips_write(self) -> None:
        ...
        # First call: writes
        ref1 = projector.project(target_profile=RAGFLOW_RECALL_PROFILE, document=doc)
        assert len(fake.upload_document_calls) == 1

        # Second call: same content -> list_documents returns the existing doc, no upload
        ref2 = projector.project(target_profile=RAGFLOW_RECALL_PROFILE, document=doc)
    ...
    def test_different_content_hash_triggers_new_upload(self) -> None:
        ...
        ref2 = projector.project(target_profile=RAGFLOW_RECALL_PROFILE, document=doc2)
