...
from agent_knowledge.couchdb_source.index_fallback import reconstruct_sessions, RETIRED_INDEX_BRIDGE_FALLBACK_STATUS
from agent_knowledge.couchdb_source.source_store import InMemoryCouchDBSourceStore


class _FakeReader:
    """Stands in for RetiredIndexBridgeReader: returns a canned session->docs map and bodies."""
...
def test_reconstruct_flags_index_fallback_and_writes_clean():
    ...
    assert pa["source"] == RETIRED_INDEX_BRIDGE_FALLBACK_STATUS
    ...
    assert sess["source_status"] == RETIRED_INDEX_BRIDGE_FALLBACK_STATUS
