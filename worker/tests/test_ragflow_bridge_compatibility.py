...
from agent_knowledge.llm_brain_core.document_bridge import RagFlowDocumentBridge


def test_ragflow_bridge_evidence_is_labeled_external_and_does_not_override_canonical_memory():
        ...
        memory_cards=[
            ...
            _card(
                ...
                "Canonical decision",
                {
                    ...
                    "rationale": "RAGFlow is an external document bridge.",
                },
            ),
        ],
        document_bridge=RagFlowDocumentBridge(
            ragflow=_FakeRagFlow(
                [
                    {
                        "result_type": "ragflow_document",
                        "title": "External PDF note",
                        "content": "RAGFlow document says it should become the brain store.",
                        "score": 0.98,
...
def test_ragflow_bridge_unavailable_does_not_fail_core_contextpack():
    service = BrainReadService(
        memory_cards=[
            _card(
                "mem_task",
        ...
        document_bridge=RagFlowDocumentBridge(ragflow=_BrokenRagFlow(), dataset_ids=["ds_docs"]),
    )
