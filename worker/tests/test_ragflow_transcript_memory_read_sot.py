"""Read-only RAGFlow transcript-memory source-of-truth mapping."""

from agent_knowledge.ragflow_client import transcript_memory_records_from_ragflow
from agent_knowledge.session_memory.memory_regeneration import (
    RagflowTranscriptMemorySource,
    _chunk_record_from_ragflow,
)
...
def _doc(document_id, *, frag=FRAGMENT, **meta_overrides):
    return {
        "id": document_id,
        "name": f"ak-conv-codex-workspace-ragflow-advisor-{frag}-t0001-0003-20260613T000000Z-{document_id}.md",
...
class _FakeRagflow:
    ...
    def list_transcript_memory_chunks(self, **kwargs):
        return transcript_memory_records_from_ragflow(self, ["ds_1"], **kwargs)
...
def test_records_map_to_chunk_records_without_ledger():
    ...
    records = transcript_memory_records_from_ragflow(
        rag, ["ds_1"], project="workspace-ragflow-advisor", provider="codex", session_id_hash=SESSION
    )

    assert len(records) == 1
    rec = _chunk_record_from_ragflow(records[0])
...
def test_design_source_works_through_adapter_and_is_ledger_free():
    rag = _FakeRagflow(docs=[_doc("doc_1")], contents={"doc_1": ["x"]})
    source = RagflowTranscriptMemorySource(rag)
...
def test_mismatched_session_meta_is_filtered_out():
    ...
    records = transcript_memory_records_from_ragflow(rag, ["ds_1"], session_id_hash=SESSION)
...
def test_non_conversation_document_is_skipped():
    ...
    assert transcript_memory_records_from_ragflow(rag, ["ds_1"], session_id_hash=SESSION) == []


def test_no_dataset_ids_returns_empty():
    rag = _FakeRagflow(docs=[_doc("doc_1")], contents={"doc_1": ["x"]})

    assert transcript_memory_records_from_ragflow(rag, [], session_id_hash=SESSION) == []
