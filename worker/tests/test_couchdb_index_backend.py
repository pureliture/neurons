"""CouchDBIndexBackendAdapter 유닛 테스트.
...
- shadow_worker wiring: INGRESS_DELIVERY_BACKEND=couchdb 환경 변수 선택 시 CouchDBIndexBackendAdapter가
...
from agent_knowledge.rag_ingress.couchdb_index_backend import (
...
SESSION_ID_HASH = sha256_hash("codex:index-backend-test-session")
...
def _ingress_payload(
    *,
    ...
    metadata에 session_id_hash / chunk_id를 포함해야 CouchDBIndexBackendAdapter가
    올바른 doc_id를 계산한다.
    """
    if body is None:
        body = _body()
...
def _adapter(store: InMemoryCouchDBSourceStore | None = None) -> CouchDBIndexBackendAdapter:
    if store is None:
...
# ---------------------------------------------------------------------------
# CouchDBIndexBackendAdapter.submit_document
# ---------------------------------------------------------------------------
...
class TestFindByNaturalKey:
    """CouchDBIndexBackendAdapter는 conservative dedup: find_by_natural_key는 항상 None 반환.
...
class TestErrorPropagation:
    def test_submit_propagates_couchdb_error(self):
        ...
        adapter = CouchDBIndexBackendAdapter(store=FailingStore())
        payload = _ingress_payload()
        document = _document_from_payload(payload)
...
class TestShadowWorkerWiring:
    """INGRESS_DELIVERY_BACKEND=couchdb 환경 변수 선택 시 CouchDBIndexBackendAdapter가 선택된다."""
    ...
    def test_env_couchdb_selects_couchdb_adapter(self, monkeypatch, tmp_path):
        ...
        build_couchdb_index_backend() 팩토리가 CouchDBIndexBackendAdapter를 반환하는지 검증한다.
        ...
        assert callable(build_couchdb_index_backend)
    ...
    def test_process_payload_deliver_true_calls_submit_document(self, tmp_path):
        ...
        adapter = CouchDBIndexBackendAdapter(store=couchdb_store)
    ...
    def test_process_payload_dedup_via_ingest_store_not_natural_key(self, tmp_path):
        ...
        CouchDBIndexBackendAdapter.find_by_natural_key는 None을 반환하므로
