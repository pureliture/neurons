...
- `@server.call_tool(validate_input=False)`: 입력 검증을 SDK가 아닌 기존 `_call_tool`
...
async def _dispatch_call_tool(
    ...
    """tool 호출의 transport 측 단일 처리부. 동기 `_call_tool`을 워커 스레드로 위임하고
    ...
    try:
        ...
        result = await run_in_threadpool(_call_tool, args, service)
