...
PROJECTION_JOB_SCHEMA_VERSION = "llm_brain_ragflow_projection_job.v1"
PROJECTION_PAYLOAD_SCHEMA_VERSION = "llm_brain_ragflow_projection_payload.v1"
PROJECTABLE_LIFECYCLE_STATES = {"accepted", "human_accepted", "auto_accepted"}
PROJECTABLE_APPROVAL_STATES = {"approved", "auto_accepted"}


class RagflowMemoryCardProjectionClient:
    """Adapter from MemoryCard projection payloads to RAGFlow document APIs."""

    def __init__(self, *, ragflow: Any, dataset_id: str):
        if not dataset_id:
    ...
    def upsert_memory_card(self, payload: Mapping[str, Any], *, idempotency_key: str) -> dict:
        ...
        existing = _find_existing_projection_document(self.ragflow, self.dataset_id, filename)
        if existing:
            return {
...
def build_ragflow_projection_payload(card: Mapping[str, Any]) -> dict:
    projectable = _require_projectable_card(card)
...
def render_projection_document(payload: Mapping[str, Any]) -> str:
    """Render a redacted, searchable document body for the RAGFlow mirror."""
...
def execute_projection_job(job: Mapping[str, Any], *, client: Any, allow_write: bool = False) -> dict:
    ...
    return {
        ...
        "canonical_state_changed": False,
        "projection_state": {"status": "fresh", "reason": "ragflow_upsert_ok"},
        "ragflow_response": response,
    }
...
def _find_existing_projection_document(ragflow: Any, dataset_id: str, filename: str) -> str:
    documents = ragflow.list_documents(dataset_id, keywords=filename, page=1, page_size=20)
...
def _projection_write_gate_error(job: Mapping[str, Any], *, client: Any) -> str:
    ...
    if str(approval.get("operation") or "") != "ragflow_projection_write":
