...
:class:`RetiredIndexBridgeAdapter`. The adapter owns *all* backend specifics:

- logical ``target_profile`` -> physical dataset id resolution,
- the concrete client call shape (upload/parse/status),
- the backend status vocabulary (RetiredIndexBridge ``DONE/FAIL/RUNNING/UNSTART``) which is
  translated into the generic :class:`IndexStatus`.

Today the only adapter is :class:`RetiredIndexBridgeRetiredIndexBridgeAdapter`. A future Qdrant /
...
class BackendStatusDetail:
    ...
    backend reported (e.g. RetiredIndexBridge ``run``) so a caller that owns a legacy ledger
...
@runtime_checkable
class RetiredIndexBridgeAdapter(Protocol):
    def submit_document(
...
# RetiredIndexBridge ``run`` vocabulary -> generic IndexStatus. This table is the single
# place the backend status words appear in the ingress bus.
_RETIRED_INDEX_BRIDGE_RUN_TO_STATUS = {
...
class RetiredIndexBridgeRetiredIndexBridgeAdapter:
    """Adapter that maps the generic document model onto RetiredIndexBridge.

    ``client`` is a ``RetiredIndexBridgeHttpClient``-shaped object. ``resolve_dataset_id``
    maps a logical ``target_profile`` to a physical RetiredIndexBridge dataset id (in
    production this reads the ledger ``index_targets`` mapping). The generic
    document already carries flat metadata, so it is forwarded as RetiredIndexBridge
    ``meta_fields`` without further RetiredIndexBridge-shaped flattening here.
    """
    ...
    def submit_document(
        ...
        # Java worker had (RetiredIndexBridgeTargetAdapter contentHash recording).
    ...
    def _map_run(run: str) -> str:
        return _RETIRED_INDEX_BRIDGE_RUN_TO_STATUS.get(str(run), IndexStatus.UNKNOWN)
