    ...
    *,
    ...
    for doc in documents:
        ...
        obj = KnowledgeObjectEnvelope.from_parts(
            ...
            lifecycle_status="current" if lane == "accepted_current" else "proposed" if lane == "proposal_only" else "observed",
            authority_lane=lane,
            verification_state="source_hash_verified" if lane == "accepted_current" else "unverified",
            review_state="accepted" if lane == "accepted_current" else "needs_review" if lane == "proposal_only" else "not_required",
            content_hash=content_hash,
            evidence_refs=evs,
