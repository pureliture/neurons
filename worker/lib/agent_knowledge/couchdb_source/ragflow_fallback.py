"""RAGFlow-fallback reconstruction for sessions whose provider originals are gone.

For sessions present in RAGFlow ``transcript-memory`` but absent from CouchDB
(originals rotated/deleted, or leak-blocked at re-import), reconstruct the
conversation source from RAGFlow's already-redacted copies. Every reconstructed
body is re-checked against the fail-closed leak gate; a session whose RAGFlow
copy still leaks is skipped (not stored). Reconstructed sessions are flagged
``source=ragflow_fallback`` with an unverified project (RAGFlow project metadata
is polluted), so the retirement gate excludes them from clean-eligibility.

This is a recovery path, not the primary migration. Run it with RAGFlow + CouchDB
...
RAGFLOW_FALLBACK_STATUS = "ragflow_fallback"


class RagflowReader:
    """Minimal RAGFlow HTTP reader (list documents + fetch chunk content)."""
    ...
    def session_doc_map(self, want: set[str], *, max_pages: int = 300) -> tuple[dict, dict]:
        """Map each wanted session_id_hash to its RAGFlow document ids + (provider, project)."""
        ...
        try:
            ...
            for i, body in enumerate(bodies):
                chunk = TranscriptChunk.from_text(
                    ...
                    turn_end_index=i,
                    text=body,
                    source_status=RAGFLOW_FALLBACK_STATUS,
                )
            ...
            session = TranscriptSession(
                ...
                started_at="", source_status=RAGFLOW_FALLBACK_STATUS,
            )
            ...
            store.put(build_coverage_manifest_document(
                ...
                project_authority={
                    "project": project, "source": RAGFLOW_FALLBACK_STATUS, "ambiguous": True,
                    "eligible_for_retirement": False,
                    "notes": ["ragflow_fallback", "ragflow_project_unverified"],
                },
            ))
...
__all__ = ["RagflowReader", "reconstruct_sessions", "RAGFLOW_FALLBACK_STATUS"]
