...
RetiredIndexBridge `transcript-memory`를 은퇴(삭제)하기 위해, 라이브 recall을 CouchDB
source에서 재생성한 RetiredIndexBridge `session-memory` surface로 무회귀 컷오버한다. 데이터
...
CouchDB source에서 M3 materializer로 session-memory 문서를 생성해 RetiredIndexBridge
...
CouchDB transcript-source (3567 sessions)
        ...
        v
SessionMemoryRebuilder ── live projector ──► RetiredIndexBridge session-memory (f6c55b)
        |                                          | parse/index
        v                                          v
...
2. 재생성: 각 CouchDB 세션 → materialize_session_memory → session-memory doc →
   RetiredIndexBridge session-memory upload + parse. 결정적 id로 idempotent.
...
- **SessionMemoryRebuilder**: 입력 CouchDB source(conversation chunk + tool
  evidence bundle); 출력 RetiredIndexBridge session-memory docs(parsed). 의존 M3
  materializer + RetiredIndexBridge upload/parse. 실패 시 부분 진행 + 재실행 안전.
- **RecallSmoke**: 입력 대표 쿼리 + 두 dataset; 출력 per-query hits/유사도 +
  무회귀 verdict. 의존 RetiredIndexBridge retrieval API.
...
- **WriteCutover**: 입력 delivery 설정; 출력 transcript-memory write 0. 의존
  RETIRED_INDEX_BRIDGE_DELIVERY/dataset 설정.
...
- 통합: 소량 세션 재생성 → RetiredIndexBridge parse → retrieval hit 확인(라이브 소규모).
- 라이브 게이트: recall smoke 무회귀, 재배선 후 recall 출처 확인, 백업 무결성,
  삭제 후 recall 지속.

## Milestones

- RC1: session-memory 0-hits 진단 + CouchDB에서 session-memory 재생성 → RetiredIndexBridge
