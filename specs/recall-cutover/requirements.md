...
- 실행 중 recall은 `neuron-knowledge mcp-stdio --ragflow-direct-recall
  --dataset-id <transcript-memory>` 로 **transcript-memory(cca3d0)를 직접** 읽는다.
- session-memory(f6c55b, 6668 docs)는 RAGFlow retrieval에서 대표 쿼리 4개 모두
  **0 hits** (현재 recall surface로 동작 안 함).
- CouchDB transcript-source에는 3567 세션(clean 3326 + ragflow_fallback 241)이
  적재·검증 완료. 14 세션은 leak-blocked로 저장 불가(백업 전용).
- transcript-memory write(delivery)는 `RAGFLOW_DELIVERY_ENABLED=true`로 활성.
...
RAGFlow `session-memory`(f6c55b)다.

- design.md 계약: RAGFlow `session-memory`가 brain-facing recall surface이고
...
- 따라서 CouchDB-direct recall이나 transcript-memory 유지가 아니라, CouchDB
  source에서 **derived session-memory**를 RAGFlow에 구축해 recall을 받는다.
...
- 컷오버 done 상태: session-memory가 CouchDB source(conversation + tool evidence
  요약)에서 재생성되어 **RAGFlow retrieval로 hit가 나오는** 상태여야 한다.
...
- `RAGFLOW_DELIVERY`의 transcript-memory 타깃을 비활성/제거한다.
...
- CouchDB source(3567 세션)에서 RAGFlow `session-memory`를 재생성하고 retrieval
