...
dedup을 한다: (1) exact-dup 제거, (2) **subsumption** — 더 긴 chunk가 짧은 chunk의 turn 윈도우를
...
- 대안 기각 1: Java RAGFlow `supersedePriorVersions`(기본 off, `logical_document_id` 필요) —
  RAGFlow projection 레이어만 정리하고 **CouchDB M3 substrate는 그대로 중복**이라 부적합.
- 대안 기각 2: MemoryCard supersede(`supersedes`/`currentness`) — 그건 brain card 레이어이고
  session-memory chunk 레이어가 아니라 층위가 다름.

### Q: 어떤 규칙인가?

regen 경로와 **동일한 정책**: 같은 세션의 chunk들 중
- exact-dup(같은 content_hash + 동일 turn/part/char 범위) → 1개만 유지.
- subsumption: 더 긴 chunk가 짧은 chunk의 turn 윈도우를 strict 포함하고 sanitized text도 포함하면
...
live RAGFlow mutation/GC 없음. 원본 삭제 없음. Hermes #48(provider identity/steward)와 정합 유지.
...
- coverage 게이트(`materialization_loss`/`fully_materialized`) 동작을 바꾸지 않는다(저장 수 기준 유지).
...
| Non-destructive | CouchDB 원본 chunk 삭제/수정 금지(in-memory canonicalization만). live RAGFlow/GC 금지. |
...
- L3(별도 승인): 실제 CouchDB/RAGFlow 런타임에서의 재투영 검증.

## 허용 / 금지 범위

- 허용: M3 materializer in-memory canonicalization(subsumption+exact-dup), 공유 pure 함수 추출,
  합성 fixture 테스트.
- 별도 승인: 실제 런타임 재투영, live RAGFlow write/delete.
- 금지: CouchDB 원본 chunk 삭제/수정, live RAGFlow/GC mutation, Java/ingress kind 계약 변경,
  dendrite 동작 변경(불필요).
