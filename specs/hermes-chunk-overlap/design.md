...
- 핵심: canonical M3에서 in-memory subsumption(긴 chunk가 짧은 chunk의 turn window+텍스트를
  포함하면 drop) + exact-dup 제거 후 concat. coverage gate 불변, 원본 비삭제, dendrite 무변경,
  worker Python 한정.
...
CouchDB chunk schema에 `supersedes` 추가 + 새 chunk가 옛 chunk를 포함하면 옛 것을 superseded로
표시, materialize/find_by_session가 필터. 스키마+쓰기경로+필터 신설로 범위가 크고, 원본 상태를
변경(가역성↓). M3 dedup만으로 recall이 깨끗해지므로 과함.

### 선택안 C (기각): Java RetiredIndexBridge `supersedePriorVersions` 활성화

projection layer(RetiredIndexBridge)만 정리. 기본 off + `logical_document_id` 필요. **CouchDB M3 substrate는
...
flowchart LR
  ...
  BODY --> PROJ[project (RetiredIndexBridge/Qdrant)]
...
sequenceDiagram
  ...
  Note over M: 저장 수는 불변 → coverage gate 영향 없음
...
- 2단계 subsumption: chunk A가 chunk B의 turn window를 strict 포함(A.turn_start<=B.turn_start,
...
| 부분 겹침(turn window 교차하나 포함 아님) | subsumption 미적용(보수적). 둘 다 유지(기존 동작). |
