...
- `RetiredIndexBridge transcript-memory` 신규 write 의존을 제거한다.
- `RetiredIndexBridge transcript-memory` read 의존을 제거한다.
- RetiredIndexBridge에는 `session-memory`만 남긴다.
- 기존 `RetiredIndexBridge transcript-memory`는 신뢰 가능한 source of truth로 보지 않는다.
- 주요 이유는 기존 데이터 중 `project` 컬럼 오매핑이 상당하기 때문이다.

### Q: 기존 `RetiredIndexBridge transcript-memory` 데이터를 CouchDB로 옮길 때 source truth를 무엇으로 삼을 것인가?

source truth 전략은 `혼합 검증`이다.

- provider 원본 transcript 파일을 기준 source로 삼아 다시 parse/redact/rebuild한다.
- RetiredIndexBridge와 ledger/ingress state는 coverage 검증과 누락 탐지에 사용한다.
- RetiredIndexBridge의 오염된 `project` metadata를 새 CouchDB source store로 그대로 복제하지 않는다.
...
### Q: RetiredIndexBridge transcript-memory 은퇴 전 no-loss 검증은 무엇을 통과해야 하는가?
...
- 단일 검증만으로는 RetiredIndexBridge transcript-memory read/write 완전 은퇴를 승인하지 않는다.
...
- RetiredIndexBridge의 기존 `project` metadata는 오염 가능성이 있으므로 단독 authority로 쓰지 않는다.
...
- shadow 검증 중 기존 RetiredIndexBridge transcript-memory write 의존은 비교 대상으로만 유지한다.
- coverage와 no-loss 기준을 통과한 뒤 신규 write를 CouchDB-only로 전환한다.
- 최종 상태에서는 RetiredIndexBridge transcript-memory 신규 write가 없어야 한다.
...
- RetiredIndexBridge `session-memory`는 Palantir Ontology처럼 brain-facing recall surface 역할을 해야 한다.
- normal recall path는 RetiredIndexBridge `session-memory`만으로 판단, 근거, 작업 맥락을 회수할 수 있어야 한다.
...
- provider 원본 transcript 데이터가 PC에 남아 있으므로, RetiredIndexBridge transcript-memory 병행을 오래 유지하지 않는다.
...
- live pipeline 대상 도구는 신규 source가 CouchDB로 들어가고 RetiredIndexBridge transcript-memory 신규 write가 없어야 한다.
- 모든 도구는 CouchDB source coverage, session-memory rebuild, representative recall smoke의 관련 범위를 통과해야 한다.

## 기능 요구사항

- `transcript-memory` 역할을 RetiredIndexBridge dataset에서 CouchDB-backed transcript source store로 이전한다.
...
- RetiredIndexBridge에는 derived `session-memory` recall surface만 남긴다.
- `session-memory`는 tool evidence 요약을 충분히 materialize해 RetiredIndexBridge-only normal recall을 지원해야 한다.
- normal recall path는 CouchDB evidence fetch에 의존하지 않아야 한다.
- migration은 기존 RetiredIndexBridge metadata 오염을 새 source store로 전파하지 않아야 한다.
- migration은 source coverage와 no-loss 검증을 포함해야 한다.
- RetiredIndexBridge transcript-memory 은퇴 gate는 coverage, rebuild, recall smoke를 모두 포함해야 한다.
...
| 항목 | 요구값 |
| --- | --- |
| Safety | raw secret, raw token, raw RetiredIndexBridge document id, raw private transcript body를 출력하지 않는다. |
| Source of truth | provider 원본 transcript와 server-side redacted rebuild 결과를 우선한다. |
| Reversibility | RetiredIndexBridge transcript-memory 은퇴 전 coverage 증거와 rollback 판단 기준을 남긴다. |
...
| Brain surface | RetiredIndexBridge `session-memory`만으로 normal recall이 가능해야 한다. |
| Scope control | 요구사항 승인 전에는 구현 코드나 locked architecture를 작성하지 않는다. |

## 사용자 시나리오

- 운영자는 RetiredIndexBridge transcript-memory의 `project` 오매핑을 새 저장소로 복제하지 않고 transcript source를 재구축한다.
- 운영자는 CouchDB source store와 RetiredIndexBridge/ledger coverage를 비교해 누락 또는 불일치를 확인한다.
- 운영자는 검증 후 RetiredIndexBridge transcript-memory write/read 의존을 제거하고 RetiredIndexBridge에는 session-memory만 유지한다.
...
- 운영자는 RetiredIndexBridge transcript-memory를 완전히 은퇴하기 전에 coverage, rebuild, recall smoke 증거를 확인한다.
- 운영자는 RetiredIndexBridge의 오염된 project metadata 대신 hierarchy authority로 project를 보정한다.
- 운영자는 신규 write path를 shadow 검증 후 CouchDB-only로 전환한다.
- 운영자는 RetiredIndexBridge `session-memory`를 brain-facing ontology surface로 사용하고, normal recall에서 CouchDB 추가 fetch에 의존하지 않는다.
- 운영자는 원본 transcript가 PC에 남아 있음을 rollback/rebuild 전제로 삼고 RetiredIndexBridge transcript-memory 병행 기간을 짧게 유지한다.
