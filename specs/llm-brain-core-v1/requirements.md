...
이 요구사항은 `neurons` repo 안에 LLM-Brain Core v1을 설계하기 위한 Phase 1 source다. 목표는 기존 `dendrite`/`neurons` 세션 수집 파이프라인을 유지하면서, RetiredIndexBridge 중심 recall surface를 장기 기억 core로 승격하지 않고, 세션·작업·결정·장애·페르소나·파일 참조를 다루는 로컬 우선 brain memory 계층을 만드는 것이다.
...
답: v1은 `neurons`에 구현한다. 현재 `neurons`가 server-side brain authority이며, `MemoryCard`, ledger, `brain_query`, candidate mining, RetiredIndexBridge projection mirror 관련 코드가 이미 있다. 새 repo 분리는 graph-core vertical slice와 per-PC/central mode가 안정화된 뒤 검토한다.

### Q3. RetiredIndexBridge는 core인가?

답: 아니다. 기존 Ubuntu RetiredIndexBridge는 보존하지만, LLM-Brain Core v1의 canonical memory, ontology store, persona DB, default vector DB로 쓰지 않는다. RetiredIndexBridge는 기존 corpus와 PDF/PPT/Excel/scan/citation 같은 complex document class를 위한 external bridge로만 둔다.
...
답: OpenClaw, Codex, Claude Code 같은 runtime agent가 `Graphiti`, `Neo4j`, `CouchDB`, RetiredIndexBridge를 직접 알면 기술 교체와 safety boundary가 깨진다. v1은 새 gateway 제품을 만들지 않고, `neurons` 내부 read-oriented brain API와 thin MCP/stdio adapter만 제공한다.

### Q7. 기존 llm-brain/autopilot 개발은 버리는가?

답: 버리지 않는다. `MemoryCard`, candidate mining, approval/supersession, ledger write, brain query read-model 아이디어는 재사용한다. 다만 `autopilot_cli.main`, live RetiredIndexBridge mining, self-minted RetiredIndexBridge projection approval, `index_projection.py` write path는 bridge 단계 전까지 core 재사용 대상에서 제외한다.

### Q8. 가장 큰 기술 부채는 무엇인가?

답: RetiredIndexBridge projection mirror와 canonical memory 역할이 코드와 문서에서 섞여 있는 점이다. 이 부채는 앞으로 모든 brain 기능 변경을 RetiredIndexBridge availability, dataset shape, parse state에 묶이게 한다. v1 설계는 이 coupling을 명시적으로 끊어야 한다.
...
- RetiredIndexBridge는 bridge/fallback으로만 호출되어야 하며 core 기능의 required dependency가 아니어야 한다.
...
| Incrementality | 기존 RetiredIndexBridge와 session-memory runtime을 즉시 삭제하지 않고 bridge로 보존한다. |
| Testability | RetiredIndexBridge 없이도 core acceptance test가 통과해야 한다. |
...
- 사용자가 RetiredIndexBridge 없이 local brain을 띄워도 AI session-memory, Task/Decision, Incident, PersonaFact, ContextPack은 동작한다.
- 사용자가 기존 RetiredIndexBridge corpus가 필요한 질문을 하면 RetiredIndexBridge bridge가 document/citation fallback으로만 호출된다.

## Acceptance Gates

- RetiredIndexBridge disabled 상태에서 latest work ContextPack test가 통과한다.
- RetiredIndexBridge disabled 상태에서 Decision drift timeline test가 통과한다.
- RetiredIndexBridge disabled 상태에서 Incident search/replay test가 통과한다.
- RetiredIndexBridge disabled 상태에서 PersonaFact candidate/check test가 통과한다.
...
- RetiredIndexBridge bridge를 켜도 canonical memory winner는 `session-memory` artifact와 MemoryCard ledger이며, graph store는 derived index, RetiredIndexBridge는 bridge lag로만 취급된다.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | RetiredIndexBridge projection naming이 core memory처럼 읽힘 | Docs/knowledge | `README.md`, `specs/recall-cutover/*`, `worker/lib/.../index_projection.py` | 설계자가 RetiredIndexBridge를 canonical brain으로 오해함 | 5 | 4 | 2 | 36 |
...
| 2 | Graph-backed memory adapter seam 부재 | Design/architecture | `session_memory/*` | Graphiti/Neo4j 도입 시 RetiredIndexBridge path를 우회하기 어려움 | 5 | 4 | 3 | 27 |
...
- RetiredIndexBridge projection은 bridge/fallback으로 격하한다.
...
- RetiredIndexBridge write/delete/disable은 ordinary autopilot 범위가 아니다.
