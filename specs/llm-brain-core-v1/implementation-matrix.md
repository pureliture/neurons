...
| Build latest-work ContextPack | implemented | `test_llm_brain_core_ragflow_disabled.py`, `test_llm_brain_core_runtime_integration.py`, `test_ontology_episode_batch.py`; graph-only task fallback is covered | Live Graphiti/Neo4j ranking smoke deferred until Docker daemon and Compose plugin are available |
| Incident/troubleshooting search | implemented with graph contract | `BrainReadService.brain_incident_search`; runtime incident tests pass; `GraphProjectionWorker` projects ontology episodes through the graph adapter interface | Live Graphiti/Neo4j replay smoke deferred until Docker daemon and Compose plugin are available |
| Time-aware drift explanation | implemented from cards and graph contract | `BrainReadService.brain_drift_explain`; runtime drift tests pass; graph-backed ontology projection tests pass | Live Graphiti/Neo4j temporal smoke deferred until Docker daemon and Compose plugin are available |
...
| M5 Incident, drift, persona | done with graph contract | Incident/drift/persona tests pass; ontology projection covers graph adapter input | Live Graphiti/Neo4j smoke is deferred with environment proof |
