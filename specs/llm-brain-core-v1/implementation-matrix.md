...
| Metadata-first hybrid graph transition | local contract implemented | `MetadataFirstHybridGraphAdapter` stores metadata-first episodes in any `GraphMemoryAdapter`, keeps free-text recall in optional `HybridTextMirror`, and joins public-safe hints through `get_episodes_by_ids`; `test_metadata_first_hybrid_graph.py` verifies metadata-only graph payload, scope preservation, exact episode-id join, hybrid search, and ContextPack restore | Runtime builder wiring and production Qdrant/Docling mirror integration are not claimed here; they remain separate follow-up work |
