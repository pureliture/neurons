...
| `neurons` owns session-memory artifact and graph projection | implemented and Ubuntu live-smoked | `LedgerSessionMemoryArtifactStore` stores artifacts; `GraphProjectionWorker` projects artifacts, MemoryCards, and SourceRefs to `GraphMemoryAdapter`; Ubuntu Graphiti/Neo4j smoke returned `status=available`, `details=["graphiti_neo4j"]`, `episode_count=2`, `matched=true` | None for v1 graph projection contract |
...
| M6c Graphiti/Neo4j Ubuntu integration | done with live smoke | `GraphitiNeo4jGraphMemoryAdapter`, `GraphProjectionWorker`, and `llm-brain-neo4j` compose profile exist; Ubuntu Neo4j container is healthy; adapter smoke returned `status=available`; dump/load returned `episodes=3` | Keep destructive graph DB operations out of autopilot |
