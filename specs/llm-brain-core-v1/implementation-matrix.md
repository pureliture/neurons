...
| Metadata-first hybrid graph transition | runtime wiring implemented | `MetadataFirstHybridGraphAdapter` stores metadata-first episodes in any `GraphMemoryAdapter`, keeps free-text recall in optional `HybridTextMirror`, and joins public-safe hints through `get_episodes_by_ids`; `build_graph_adapter_from_env` selects it with `LLM_BRAIN_GRAPH_METADATA_FIRST_HYBRID=true` and forces Graphiti entity extraction off for that path; tests verify metadata-only graph payload, scope preservation, exact episode-id join, hybrid search, ContextPack restore, and runtime env wiring | Production persistent text mirror integration is not claimed here; it remains separate follow-up work |
...
```text
Ubuntu checkout: /home/ops/Projects/neurons
...
Host Python still lacks Graphiti runtime dependencies, so current-pass graph
verification used an isolated container runner built from the checked-out worker
tree instead of installing host packages.

Current Ubuntu containerized Graphiti/Neo4j adapter smoke:

```json
{
  "status": "available",
  "details": ["graphiti_neo4j", "episodic_node_direct_read"],
  "commit": "8b9596d",
  "matched": true,
  "labels": ["Episodic"],
...
Current Ubuntu graph dump/load smoke:

```json
{
  "status": "ok",
  "episodes": 4,
  "dump_file_present": true,
  "restore_volume_created": true,
  "graph_db_files_synced_between_pcs": false
}
```
...
```bash
ssh ops-host 'docker inspect -f "{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}" llm-brain-verify-1972ee1-llm-brain-neo4j-1'
```
