...
transcript-memory(RAGFlow) 은퇴 마이그레이션으로 raw transcript의 SoT가 CouchDB로
...
- **B. CouchDB → session-memory builder**: CouchDB에서 dirty/미projection 세션을 읽어
  `materialize_and_project`로 RAGFlow session-memory(db831f) 카드 생성.
...
서비스 대신 이 consume 경로의 **sink를 transcript-memory(RAGFlow)에서 CouchDB
...
materializer로 교체. RAGFlow/PG compose 미수정 원칙 유지(host-published endpoint).
...
| 자격증명 | `RAGFLOW_API_KEY` 단일 |
| RAGFlow 변경 | RAGFlow/PG compose 미수정, host-published endpoint만 사용 |
